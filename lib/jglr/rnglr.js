define(["./cyclicJSON"], function(cycle) {
      ...
      if (findBySymbols) {
        ...
        if (findByLookahead) {
          ...
          if (findByPos) {
            ...
            if (findByAction) {
              var findByFlags = findByAction[JSON.stringify(flags)];
              if (findByFlags) return findByFlags;
              else {
                var rule = new Rule(name, symbols, lookahead, pos, action, flags);
                RuleFactory.byId[rule.id] = rule;
                findByAction[JSON.stringify(flags)] = rule;
                return rule;
              }
            } else {
              var rule = new Rule(name, symbols, lookahead, pos, action, flags);
              RuleFactory.byId[rule.id] = rule;
              var byFlags = {}; byFlags[JSON.stringify(flags)] = rule;
  ...
  Rule.fromSerializable = function(rulesByOldId, id) {
    ...
    if (obj.hasOwnProperty("like")) {
      ...
      return RuleFactory.make(obj.name, sym, Atom.fromSerializable(obj.lookahead), obj.position, eval(obj.action), obj.flags ? JSON.parse(obj.flags) : undefined);
    }
  }
  ...
  GSSNode.prototype.forPathsToSet = function(setId, callback) {
    if (this.parentSetId === setId) {
      callback({labels: [], stack: [], leftSib: this});
    } else {
      for (var i = 0; i < this.links.length; i++)
        pathSetHelp(this.links[i], setId, [], [this.toString()], callback);
    }
  }
  function pathSetHelp(link, setId, labels, stack, callback) {
    labels.push(link.val);
    stack.push(link.prev.toString());
    if (link.prev.parentSetId === setId) {
      callback({labels: labels, stack: stack, leftSib: link.prev});
    } else {
      var prevLinks = link.prev.links;
      for (var i = 0; i < prevLinks.length; i++)
        pathSetHelp(prevLinks[i], setId, labels, stack, callback);
    }
    stack.pop();
    labels.pop();
  }
    ...
    parseRNGLR: function(token_source) {
      if (token_source.isEmpty()) {
        ...
        try {
          ...
          //   var acc = undefined;
          //   for (var j = 0; j < this.U[i].size(); j++) {
          //     var head = this.U[i].get(j);
          //     head.forPathsToSet(0, function(p) {
          //       console.log("Stack in progress: [\n  " + p.stack.map(function(s) { return s.toString(); }).join(",\n  ") + "]");
          //       console.log("Labels in progress: [\n  " + p.labels.map(function(s) { return s.toString(); }).join(",\n  ") + "]");
          //     });
    ...
    plausibleTokensForState: function(state, ret) {
      ...
      for (var j = 0; j < this.tokens.size(); j++) {
        ...
        if (tok !== EPSILON && actions !== undefined && 
          ...
          var str = "";
          for (var key in actions.reductions.elements) {
            if (str !== "") str += ", ";
            str += actions.reductions.elements[key].toString();
          }
          console.log("In state " + state + " for token " + tok + ", actions = {" + str + "}");
          ret[tok] = true;
        }
      }
      return ret;
