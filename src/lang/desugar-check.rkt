...
(define (desugar-check/internal ast)
  ...
  (define (ds-bind b)
    (match b
     [(s-bind s shadow name ann) (s-bind s shadow name (ds-ann ann))]))
      ...
      [(cons? ds-stmts)
       ...
       (s-block s
        (append
          ...
          (list
              (s-let s (s-bind s #f id-result (a-blank)) last-expr)
              do-checks
              (s-id s id-result))))])]
  ...
  (match ast
    ...
    [(s-prog s imports (s-block s2 stmts))
     ...
     (define with-checks
      (cond
        [ok-last
         (define bind-result (s-let s (s-bind s #f result-id (a-blank)) last-stmt))
         (define new-stmts
          (append all-but-last (list bind-result (s-id s result-id) (s-id s 'nothing))))
          (desugar-check/internal (s-block s2 new-stmts))]
        [else
         (define bind-result (s-let s (s-bind s #f result-id (a-blank)) (s-id s2 'nothing)))
