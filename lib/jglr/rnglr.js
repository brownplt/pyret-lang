define(["./cyclicJSON"], function(cycle) {
  ...
  Rule.fromSerializable = function(rulesByOldId, id) {
    ...
    if (obj.hasOwnProperty("like")) {
      ...
      return RuleFactory.make(base.name, base.symbols, Atom.fromSerializable(lookahead), base.position);
    } else {
      var sym = [];
  ...
  Grammar.prototype = {
    ...
    getAction: function(rule) {
      var baseRule = rule.withLookahead(undefined).withPosition(0);
    ...
    getFlags: function(rule) {
      return this.flags[rule.withLookahead(undefined).withPosition(0).id];
    },
      ...
      for (var i = 0; i < rule_set.size(); i++) {
        ...
        if (rule.position < rule.symbols.length) {
          if (rule.symbols[rule.position].toString() == symbol) {
            var new_rule = RuleFactory.make(rule.name, rule.symbols, undefined, rule.position + 1);
            ...
            for (var idx = 0; idx < ntFirst.size(); idx++) {
              ...
              for (var j = 0; j < rules_a.length; j++) {
                ...
                if (rule_a.symbols.length > 0 && rule_a.symbols[0].toString() == symbol) {
                  ...
                  var new_rule = RuleFactory.make(rule_a.name, rule_a.symbols, undefined, 1);
                  ret.add(new_rule);
                }
              }
            }
