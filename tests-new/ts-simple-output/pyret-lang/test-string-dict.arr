...
import option as O
import sets as S
include from L: fold, range, list, each, find end
include from O: data Option end
include from S: tree-set end
include from G: to-repr, js-to-string, not end
...
#|
  ...
  for each(i from range(0, 100)) block:
    sd-many-as.set-now("a" + js-to-string(i), i)
    when not(i == 54):
      sd-almost-many-as.set-now("a" + js-to-string(i), i)
    end
  end
...
check "remove and unfreeze":
  ...
  d1 = d0.remove("A")
  d2 = d1.unfreeze()
  first-key = d2.keys-list-now().get(0)
...
#| TODO(joe/ben): full torepr
check "cyclic":
  s1 :: SD.MutableStringDict<Any> = [SD.mutable-string-dict: {"a"; nothing}]
  s1.set-now("a", s1)
  to-repr(s1) is "[mutable-string-dict: \"a\", <cyclic-object-1>]"
end
|#
