...
R(['jglr/jglr', 'fs'], function (E, fs) {
  ...
  function generateAlts(ruleName, alts, shouldInline) {
    ...
    if (alts.kids.length === 1) {
      ...
      if (shouldInline) {
        if (gen.flags) {
          ret.rules.push("{\"name\": " + JSON.stringify(ruleName) + ", \"symbols\": [" + gen.rhs + "], "
            + JSON.stringify(gen.flags) + "},"); // TODO Rule Inline
        }
        else
          ret.rules.push("{\"name\": " + JSON.stringify(ruleName) + ", \"symbols\": [" + gen.rhs + "]},"); // TODO Rule Inline
      } else {
        if (gen.flags) {
  ...
  function generateItem(ruleName, item) {
    if (item.name === "Name") {
      ...
      ret.rules.push("{\"name\": "  + json_newNameStar + ", \"symbols\": []},"); // TODO Rule Inline
      ret.rules.push("{\"name\": "  + json_newNameStar + ", \"symbols\": [" +
        json_newNameStar + ", " + json_newNameOne + "], " + "\"postprocess\": function arrpush(d) {return d[0].concat([d[1]]);}},"); // TODO Rule ListSnoc
      if (item.kids[0].name === "Alts") {
        ...
        ret.rules = ret.rules.concat(gen.rules);
        ret.rules.push("{\"name\": " + JSON.stringify(newNameOne) + ", \"symbols\": [" + gen.rhs + "]},"); // TODO Rule Inline
      }
      ...
      ret.rules.push("{\"name\": " + json_newNamePlus + ", \"symbols\": " +
        "[" + json_newNameOne + "]},"); // TODO Rule Inline
      ret.rules.push("{\"name\": " + json_newNamePlus + ", \"symbols\": " +
        "[" + json_newNamePlus + ", " + json_newNameOne + "], " + "\"postprocess\": function arrpush(d) {return d[0].concat([d[1]]);}},"); // TODO Rule ListSnoc
      if (item.kids[0].name === "Alts") {
        var gen = generateAlts(newNameOne, item.kids[0], true);
        ret.rules = ret.rules.concat(gen.rules);
      } else {
      ...
      ret.rules.push("{\"name\": " + json_newNameOpt + ", \"symbols\": []},"); // TODO Rule Inline
      ret.rules.push("{\"name\": " + json_newNameOpt + ", \"symbols\": [" + json_newNameOne + "]},"); // TODO Rule Inline
      var gen = generateAlts(newNameOne, item.kids[0], true);
      ret.rules = ret.rules.concat(gen.rules);
  ...
  if (parsed !== undefined) {
    ...
    out.write("R.config({paths: {'jglr': '" + jglrRunPath + "', 'nearley': '.'}});\n\n");
