...
function makeModuleFinder(documents: Map<string, DocumentManager>, runtime: Runtime) {
	return runtime.makeFunction((ctxt: CLIContext, dep: Dependency) => {
		...
		// we have to add the protocol here because lsp uses uri while locators use path
		const fileLocator = FL['mockable-file-locator'].app(runtime.makeObject({
			...
			'file-times': null,
			'mtimes': runtime.makeFunction((p: string) => documents.get('file://' + path.resolve(p))!.mtime),
			'real-path': null,
...
export async function analyzeFile(filename: string, documents: Map<string, DocumentManager>, runtime: Runtime) {
	...
	const options = TCO['populate-options'].app(optMap, './');
	...
	const wlist = listToArray(await pyretCall(runtime, () => TCL['compile-worklist'].app(moduleFinder, base.dict.locator, base.dict.context)));
