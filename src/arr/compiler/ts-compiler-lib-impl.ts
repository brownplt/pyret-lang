...
({
  ...
  theModule: function(runtime, _, __, tj : TJ.Exports, E: EitherExports, SHA: ShaExports) {
    const { 
    ...
    function makeStandalone(worklist: List<ToCompile>, compiled: CompiledProgram, options) {
      ...
      const natives = wl.flatMap((item) => {
        const natives = listToArray(item.dict.locator.dict['get-native-modules'].full_meth(item.dict.locator));
        return natives.map((native) => native.dict.path);
      });
      const allCompileProblems = [];
      const staticModules = ObjectExpression(
        wl.map((w) => {
          const uri = w.dict.locator.dict.uri.full_meth(w.dict.locator);
          ...
          switch(loadable.$name) {
            case 'module-as-string': {
              ...
              switch(rp.$name) {
                case 'ok': {
                  return Property(w.dict.locator.dict.uri.full_meth(w.dict.locator),
                    Literal(rp.dict.code.dict['pyret-to-js-runnable'].full_meth(rp.dict.code)));
                }
                case 'err': {
                  allCompileProblems.push(...listToArray(rp.dict.problems));
                  return Property(w.dict.locator.dict.uri.full_meth(w.dict.locator), Literal("error"));
                }
                default:
      ...
      if (allCompileProblems.length > 0) {
        ...
        const toLoad = ArrayExpression(wl.map((w) => Literal(w.dict.locator.dict.uri.full_meth(w.dict.locator))));
