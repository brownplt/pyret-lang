...
if (STICKY_REGEXP === 'y') {
  Tokenizer.prototype.positionTokenRegexp = function(tok) { tok.lastIndex = this.pos; };
  Tokenizer.prototype.updateString = function(match) { };
  Tokenizer.prototype.updatePosition = function(tok, match) { this.pos = tok.lastIndex; };
} else {
  ...
  while (this.hasNext()) { // Surround the tokenizer loop...
    ...
    for (var i = 0; i < this.Tokens.length; i++) {
      ...
      if (match !== null) {
        ...
        this.updatePosition(tok.val, match);
        this.curLine += lines;
        if (lines === 0)
...
exports.STICKY_REGEXP = STICKY_REGEXP;
