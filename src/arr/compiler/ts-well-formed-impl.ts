...
({
  ...
  theModule: function (runtime, _, __, tj: TJ.Exports, Ain: (A.Exports), Cin: (C.Exports), EDin: (ED.Exports), Sin: (S.Exports)) {
    ...
    function checkWellFormed(ast: A.Program, options: CompileOptions): C.CompileResult<A.Program> {
      ...
      function wrapVisitAllowSMethod(visitor, target: A.Expr | A.Provide | A.Member | A.Bind | A.Name | A.LetBind | A.LetrecBind | A.IfBranch | A.IfPipeBranch | A.CasesBranch | A.ForBind | A.LoadTableSpec, allow: boolean): void {
        let curAllow = allowSMethod;
        allowSMethod = allow;
        visit(visitor, target);
        allowSMethod = curAllow;
      ...
      const wellFormedVisitor: TJ.Visitor<A.Program | A.ImportType | A.Ann | A.Expr | A.Member | A.LetBind | A.Bind | A.Import | A.Provide | A.ProvideTypes | A.LetrecBind | A.CasesBranch | A.IfBranch | A.IfPipeBranch | A.ForBind | A.VariantMember | A.AField, void> = {
        ...
        's-cases-branch': (visitor, expr: TJ.Variant<A.CasesBranch, 's-cases-branch'>) => {
          let oldPbl = parentBlockLoc;
          parentBlockLoc = expr.dict.l;
          if (expr.dict.name === '_') {
          ...
          const args = listToArray(expr.dict.args);
          ensureUniqueIdsOrBindings(args.map(a => a.dict.bind), false);
          args.forEach(a => visit(visitor, a));
          wrapVisitAllowSMethod(visitor, expr.dict.body, false);
          parentBlockLoc = oldPbl;
        },
        's-singleton-cases-branch': (visitor, expr: TJ.Variant<A.CasesBranch, 's-singleton-cases-branch'>) => {
          let oldPbl = parentBlockLoc;
          parentBlockLoc = expr.dict.l;
          if (expr.dict.name === '_') {
            addError(C['underscore-as-pattern'].app(expr.dict['pat-loc']));
        ...
        's-dot': (visitor, expr: TJ.Variant<A.Expr, 's-dot'>) => {
          ...
          wrapVisitAllowSMethod(visitor, expr.dict.obj, false);
        },
        ...
        's-check': (visitor, expr: TJ.Variant<A.Expr, 's-check'>) => {
          let oldPbl = parentBlockLoc;
          parentBlockLoc = expr.dict.l;
          ...
          if (!expr.dict['keyword-check']) {
            wrapVisitCheck(visitor, optionBody);
            wfExamplesBody(body);
          } else {
            wrapVisitCheck(visitor, optionBody);
        ...
        's-when': (visitor, expr: TJ.Variant<A.Expr, 's-when'>) => {
          ...
          wrapVisitAllowSMethod(visitor, expr.dict.test, false);
          wrapVisitAllowSMethod(visitor, expr.dict.block, false);
          parentBlockLoc = oldPbl;
        },
        's-if': (visitor, expr: TJ.Variant<A.Expr, 's-if'>) => {
          ...
          branches.forEach(b => wrapVisitAllowSMethod(visitor, b, false));
        },
        's-if-else': (visitor, expr: TJ.Variant<A.Expr, 's-if-else'>) => {
          ...
          branches.forEach(b => wrapVisitAllowSMethod(visitor, b, false));
          wrapVisitAllowSMethod(visitor, expr.dict._else, false);
          parentBlockLoc = oldPbl;
        },
        ...
        's-cases': (visitor, expr: TJ.Variant<A.Expr, 's-cases'>) => {
          ...
          visit(visitor, expr.dict.typ);
          wrapVisitAllowSMethod(visitor, expr.dict.val, false);
          branches.forEach(b => wrapVisitAllowSMethod(visitor, b, false));
          parentBlockLoc = oldPbl;
        },
        ...
        's-for': (visitor, expr: TJ.Variant<A.Expr, 's-for'>) => {
          let oldPbl = parentBlockLoc;
          parentBlockLoc = expr.dict.l;
          if (!expr.dict.blocky && expr.dict.body.$name === 's-block') {
            wfBlockyBlocks(expr.dict.l, [expr.dict.body]);
          }
          wrapVisitAllowSMethod(visitor, expr.dict.iterator, false);
          listToArray(expr.dict.bindings).forEach(b => wrapVisitAllowSMethod(visitor, b, false));
          visit(visitor, expr.dict.ann);
          wrapVisitAllowSMethod(visitor, expr.dict.body, false);
          parentBlockLoc = oldPbl;
        },
        ...
        's-reactor': (visitor, expr: TJ.Variant<A.Expr, 's-reactor'>) => {
          ...
          fields.forEach(f => {
            ...
            wrapVisitAllowSMethod(visitor, f, false);
          })
        },
        's-table': (visitor, expr: TJ.Variant<A.Expr, 's-table'>) => {
          ...
          rows.forEach(row => {
            ...
            elems.forEach(e => wrapVisitAllowSMethod(visitor, e, false));
          });
        },
        's-table-extend': (visitor, expr: TJ.Variant<A.Expr, 's-table-extend'>) => {
          ...
          listToArray(expr.dict.extensions).forEach(e => {
            switch (e.$name) {
              case 's-table-extend-field': {
                wrapVisitAllowSMethod(visitor, e.dict.value, false);
                visit(visitor, e.dict.ann);
                break;
              }
              case 's-table-extend-reducer': {
                ...
                wrapVisitAllowSMethod(visitor, e.dict.reducer, false);
                visit(visitor, e.dict.ann);
                break;
              }
              default: {
        ...
        's-load-table': (visitor, expr: TJ.Variant<A.Expr, 's-load-table'>) => {
          const headers = listToArray(expr.dict.headers);
          ...
          spec.forEach(s => wrapVisitAllowSMethod(visitor, s, false));
        },
      ...
      const topLevelVisitor: TJ.Visitor<A.Program | A.Expr | A.TypeLetBind | A.Variant | A.Member | A.Bind | A.LetBind | A.Import | A.Provide | A.ProvideTypes | A.LetrecBind | A.IfBranch | A.IfPipeBranch | A.CasesBranch | A.ForBind | A.VariantMember | A.Ann | A.AField, void> = {
        's-program': (visitor, expr: TJ.Variant<A.Program, 's-program'>) => {
        ...
        's-ref': (visitor, expr: TJ.Variant<A.Expr, 's-ref'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        },
        's-when': (visitor, expr: TJ.Variant<A.Expr, 's-when'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        ...
        's-assign': (visitor, expr: TJ.Variant<A.Expr, 's-assign'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        },
        's-if-branch': (visitor, expr: TJ.Variant<A.IfBranch, 's-if-branch'>) => {
          visit<A.IfBranch>(wellFormedVisitor, expr);
        },
        's-if-pipe-branch': (visitor, expr: TJ.Variant<A.IfPipeBranch, 's-if-pipe-branch'>) => {
          visit<A.IfPipeBranch>(wellFormedVisitor, expr);
        },
        's-if': (visitor, expr: TJ.Variant<A.Expr, 's-if'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        },
        ...
        's-cases-branch': (visitor, expr: TJ.Variant<A.CasesBranch, 's-cases-branch'>) => {
          visit<A.CasesBranch>(wellFormedVisitor, expr);
        },
        's-singleton-cases-branch': (visitor, expr: TJ.Variant<A.CasesBranch, 's-singleton-cases-branch'>) => {
          visit<A.CasesBranch>(wellFormedVisitor, expr);
        },
        ...
        's-array': (visitor, expr: TJ.Variant<A.Expr, 's-array'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        },
        's-construct': (visitor, expr: TJ.Variant<A.Expr, 's-construct'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        },
        's-app': (visitor, expr: TJ.Variant<A.Expr, 's-app'>) => {
          if (expr.dict._fun.$name === 's-dot' && expr.dict._fun.dict.obj.$name === 's-id' && nameToName(expr.dict._fun.dict.obj.dict.id) === 'builtins' && expr.dict._fun.dict.field === 'trace-value') {
            wrapVisitAllowSMethod(visitor, expr.dict._fun, false);
            listToArray(expr.dict.args).forEach(a => wrapVisitAllowSMethod(visitor, a, false))
          } else {
            visit<A.Expr>(wellFormedVisitor, expr);
          }
        },
        's-prim-app': (visitor, expr: TJ.Variant<A.Expr, 's-prim-app'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        },
        's-frac': (visitor, expr: TJ.Variant<A.Expr, 's-frac'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        ...
        's-id-var': (visitor, expr: TJ.Variant<A.Expr, 's-id-var'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        },
        's-id-letrec': (visitor, expr: TJ.Variant<A.Expr, 's-id-letrec'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        },
        's-dot': (visitor, expr: TJ.Variant<A.Expr, 's-dot'>) => {
          visit<A.Expr>(wellFormedVisitor, expr);
        ...
        's-for-bind': (visitor, expr: TJ.Variant<A.ForBind, 's-for-bind'>) => {
          visit<A.ForBind>(wellFormedVisitor, expr);
        },
        's-variant-member': (visitor, expr: TJ.Variant<A.VariantMember, 's-variant-member'>) => {
          switch (expr.dict.bind.$name) {
            case 's-bind': {
              visit<A.VariantMember>(wellFormedVisitor, expr);
              break;
            }
            case 's-tuple-bind': {
              wfError(runtime.ffi.makeList([
        ...
        'a-arrow': (visitor, expr: TJ.Variant<A.Ann, 'a-arrow'>) => {
          visit<A.Ann>(wellFormedVisitor, expr);
        },
        'a-arrow-argnames': (visitor, expr: TJ.Variant<A.Ann, 'a-arrow-argnames'>) => {
          visit<A.Ann>(wellFormedVisitor, expr);
        },
        'a-method': (visitor, expr: TJ.Variant<A.Ann, 'a-method'>) => {
          visit<A.Ann>(wellFormedVisitor, expr);
        },
        'a-record': (visitor, expr: TJ.Variant<A.Ann, 'a-record'>) => {
          visit<A.Ann>(wellFormedVisitor, expr);
        },
        'a-app': (visitor, expr: TJ.Variant<A.Ann, 'a-app'>) => {
          visit<A.Ann>(wellFormedVisitor, expr);
        },
        'a-pred': (visitor, expr: TJ.Variant<A.Ann, 'a-pred'>) => {
          visit<A.Ann>(wellFormedVisitor, expr);
        },
        'a-dot': (visitor, expr: TJ.Variant<A.Ann, 'a-dot'>) => {
          visit<A.Ann>(wellFormedVisitor, expr);
        },
        'a-field': (visitor, expr: TJ.Variant<A.AField, 'a-field'>) => {
          visit<A.AField>(wellFormedVisitor, expr);
        },
      };
