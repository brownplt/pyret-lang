...
compile-handler = lam(msg, send-message) block:
  ...
  if compile-opts.lint:
    prog-name = opts.get-value("program-source")
    result = CLI.lint(opts.get-value("program-source"), opts.get-value("program"))
    cases(E.Either) result block:
      | left(errors) =>
        err-list = for map(e from errors):
          J.j-str(RED.display-to-string(e.render-reason(), tostring, empty))
        end
        d = [SD.string-dict: "type", J.j-str("lint-failure"), "data", J.j-obj([SD.string-dict: "name", J.j-str(prog-name), "errors", J.j-arr(err-list)])]
        send-message(J.j-obj(d).serialize())
      | right(_) =>
        d = [SD.string-dict: "type", J.j-str("lint-success"), "data", J.j-obj([SD.string-dict: "name", J.j-str(prog-name)])]
        send-message(J.j-obj(d).serialize())
        nothing
    end
  else:
