  ...
  function performTest(useCompiled){
    ...
    describe("Number Dictionary", function() {
      it("should have correct _plus", function() {
          ...
          //Basic
          expect(plus(zero)( zero).n).toBigEqual(0);
          expect(plus(one)(zero).n).toBigEqual(1);
          expect(plus(two)(two).n).toBigEqual(4);
          expect(plus(two)(four).n).toBigEqual(6);
          expect(plus(four)(two).n).toBigEqual(6);
          
          //Negative Numbers
          expect(plus(nfive)(two).n).toBigEqual(-3);
          expect(plus(two)(nfive).n).toBigEqual(-3);
         
          //Fractions
          expect(plus(two)(half).n).toBigEqual(2.5);

          //Chaining
          expect(plus(plus(two)(half))(plus(two)(half)).n).toBigEqual(5);
          expect(plus(plus(two)(one))(plus(twty)(seven)).n).toBigEqual(30);

          //BIG
          expect(plus(big)(big).n).toBeBig(jsnums.fromString("18000000000000000"));
      });
