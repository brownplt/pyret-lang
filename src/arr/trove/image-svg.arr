...
fun tohex(num, minwidth):
  fun help(n, hexits):
    if (n == 0):
      if hexits.length() < minwidth: help(n, "0" ^ link(hexits))
      else: hexits.join-str("")
      end
    else:
      hexit = n.modulo(16)
      next = (n - hexit) / 16
      if (hexit < 10): help(next, tostring(hexit) ^ link(hexits))
      else if hexit == 10: help(next, "A" ^ link(hexits))
      else if hexit == 11: help(next, "B" ^ link(hexits))
      else if hexit == 12: help(next, "C" ^ link(hexits))
      else if hexit == 13: help(next, "D" ^ link(hexits))
      else if hexit == 14: help(next, "E" ^ link(hexits))
      else:                help(next, "F" ^ link(hexits))
      end
    end
  end
  help(num, [])
      ...
      empty-tag =
        cases(List) attrs:
          ...
          | else => PP.group(PP.langle + PP.nest(2 * INDENT,
                PP.flow(PP.str(self.name) ^ link(attrs)) + PP.str("/>")))
        end
      open-tag =
        cases(List) attrs:
          | empty => PP.group(PP.langle + PP.str(self.name) + PP.str(">"))
...
data Image:
    ...
    as-svg(self):
      node("rect",
        [simple("width", tostring(self.width)), simple("height", tostring(self.height)),
          ...
          mixed("style", [simple("fill", tostring(self.color)), simple("fill-opacity", tostring(self.color.alpha / 255)), simple("stroke-width", "1"),
  ...
  | icolor-list-to-bitmap(pixels :: List<Color>) with:
    as-svg(self):
      node("text", [], [xtext("Not Yet Implemented")])
    end,
    bounds(self): rg(pt(0,0), pt(0,0)) end
sharing:
  to-svg(self):
