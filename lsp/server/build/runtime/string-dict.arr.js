...
var SETS = require("./sets.arr.js");
var LISTS = require("./lists.arr.js");
var RAW_ARRAY = require("./raw-array.arr.js");
// TODO(alex): valueskeleton
// var VS = get(VSlib, "values");
var $PMutableStringDictBrand = "mutable-string-dict";
var $PBrandImmutable = "immutable-string-dict";
...
function eqHelp(pyretSelf, other, selfKeys, recEq) {
    ...
    for (var i = 0; i < selfKeys.length; i++) {
        var currKey = selfKeys[i];
        if (other["has-key"]) {
            var recResult = recEq(pyretSelf["get-value"](currKey), other["get-value"](currKey));
...
//////////////////////////////////////////////////
var getISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key) {
    var missing_value = {};
    var val = pyretSelf.$underlyingMap.get(key, missing_value);
...
var getValueISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key) {
    var missing_value = {};
    var val = pyretSelf.$underlyingMap.get(key, missing_value);
    if (val === missing_value) {
...
var setISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key, val) {
    var newMap = pyretSelf.$underlyingMap.set(key, val);
    return makeImmutableStringDict(newMap);
});
var mergeISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, otherDict) {
    // keys-list returns a PyretList
    var otherKeys = otherDict["keys-list"]();
    var otherKeysArr = LISTS["to-raw-array"](otherKeys);
    if (otherKeysArr.length === 0) {
        return pyretSelf;
    }
    var newMap = pyretSelf.$underlyingMap;
    for (var i = 0; i < otherKeysArr.length; i++) {
        var currKey = otherKeysArr[i];
        newMap = newMap.set(currKey, otherDict["get-value"](currKey));
    }
    return makeImmutableStringDict(newMap);
});
var removeISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key) {
    var newMap = pyretSelf.$underlyingMap.remove(key);
    return makeImmutableStringDict(newMap);
});
var hasKeyISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key) {
    var missing_value = {};
    var val = pyretSelf.$underlyingMap.get(key, missing_value);
    if (val === missing_value) {
        return false;
...
var eachKeyISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, f) {
    RAW_ARRAY["raw-array-for-each"]["raw-array-for-each"](f, pyretSelf.$underlinyMaps.keys());
    return RUNTIME.Nothing;
});
var mapKeysISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, f) {
    var mapResult = RAW_ARRAY["raw-array-map"](f, pyretSelf.$underlyingMap.keys());
    return LISTS["raw-array-to-list"](mapResult);
});
var foldKeysISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, f, init) {
...
var keysISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
    var keys = pyretSelf.$underlyingMap.keys();
...
var keysListISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
    var keys = pyretSelf.$underlyingMap.keys();
...
var countISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
    var count = pyretSelf.$underlyingMap.size;
...
//});
var equalsISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, other, recursiveEquality) {
    if (!PRIMITIVES.hasBrand($PBrandImmutable, other)) {
        return EQUALITY.NotEqual("Different brands", pyretSelf, other);
    }
    else {
        var keys = pyretSelf.$underlyingMap.keys();
        var otherKeysLength = other.count();
...
var unfreezeISDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
    var dict = Object.create(null);
    var keys = pyretSelf.$underlyingMap.keys();
    for (var ii = 0; ii < keys.length; ii++) {
        var key = keys[ii];
        var val = pyretSelf.$underlyingMap.get(key);
        dict[key] = val;
    }
...
//////////////////////////////////////////////////
var getMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key) {
    var val = pyretSelf.$underlyingDict[key];
    if (val === undefined) {
        return OPTION.none;
    }
    else {
...
var getValueMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key) {
    var val = pyretSelf.$underlyingDict[key];
    if (val === undefined) {
        throw new Error("Key " + key + " not found");
    }
...
var setMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key, val) {
    if (pyretSelf.$sealed) {
...
var mergeMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, other) {
    for (var key in other.$underlyingDict) {
        pyretSelf.$underlyingDict[key] = other.$underlyingDict[key];
    }
    return RUNTIME.$nothing;
});
var cloneMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
...
var removeMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key) {
    if (pyretSelf.$sealed) {
        throw new Error("Cannot modify sealed string dict");
    }
...
var hasKeyMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, key) {
...
var keysMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
    var keys = Object.keys(pyretSelf.$underlyingDict);
    return SETS["tree-set"].make(keys);
});
var keysListMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
    var keys = Object.keys(pyretSelf.$underlyingDict);
    return LISTS.list.make(keys);
});
var eachKeyMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, f) {
    RAW_ARRAY["raw-array-for-each"](f, Object.keys(pyretSelf.$underlyingDict));
    return RUNTIME.$nothing;
});
var mapKeysMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, f) {
    var mapResult = RAW_ARRAY["raw-array-map"](f, Object.keys(pyretSelf.$underlyingDict));
    return LISTS.list.make(mapResult);
});
var foldKeysMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, f, init) {
    return RAW_ARRAY["raw-array-fold"](function (acc, key) {
        return f(key, acc);
    }, init, Object.keys(pyretSelf.$underlyingDict));
});
var countMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
    return RUNTIME.$makeRational(Object.keys(pyretSelf.$underlyingDict).length);
});
var toreprMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, recursiveToRepr) {
    var keys = Object.keys(pyretSelf.$underlyingDict);
    var elts = [];
    ...
    function toreprElts() {
        if (keys.length === 0) {
            ...
            var result = recursiveToRepr(pyretSelf.$underlyingDict[thisKey]);
...
//    runtime.ffi.makeList(elts));
//});
var equalsMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf, other, recursiveEquality) {
    if (!PRIMITIVES.hasBrand($PMutableStringDictBrand, other)) {
        return EQUALITY.NotEqual("Not a mutable dictionary", pyretSelf, other);
    }
    else {
        var selfKeys = Object.keys(pyretSelf.$underlyingDict);
...
var freezeMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
    var map = emptyMap();
    for (var key in pyretSelf.$underlyingDict) {
...
var sealMSDBinder = PRIMITIVES.makeMethodBinder(function (pyretSelf) {
...
function createConstImmutableStringDict(names, val) {
    var arr = LISTS["to-raw-array"](names);
    var map = emptyMap();
