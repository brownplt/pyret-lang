...
(define (block-fun-ids stmts)
  (define (stmt-id stmt)
    (match stmt
      [(s-let _ (s-bind _ id _ _) (s-lam _ _ _ _ _ _ _)) id]
      [(s-let _ (s-bind _ id _ _) (s-extend s (s-lam _ _ _ _ _ _ _) fields)) id]
      [_ #f]))
...
(define (compile-block l stmts env)
  (define (compile-stmt ast-node env)
    (match ast-node
      [(s-var s (s-bind _ id _ _) val)
        (list
          #`(r:define #,(discard-_ id) #,(compile-expr/internal val env)))]
      [(s-let s (s-bind _ id _ _) val)
...
(define (compile-expr/internal ast-node env)
  ...
  (match ast-node
    ...
    [(s-try l try (s-bind l2 id ann shadow) catch)
     (attach l
       #`(r:with-handlers
            ([p:exn:fail:pyret?
              (r:lambda (%exn)
