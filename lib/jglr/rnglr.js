define(["./cyclicJSON"], function(cycle) {
  ...
  LR1Set.prototype.addRule = function(rule) {
    ...
    ruleKey = noLookahed.toString();
    var idx = this.noLook[ruleKey];
    if (idx === undefined) {
      idx = this.ruleIdx[ruleKey] = this.coreRules.push(rule) - 1;
    }
    this.noLook[idx] = this.noLook[idx] || (rule.lookahead === undefined);
    if (rule.lookahead === undefined) {
      if (this.core[idx] === undefined)
        this.core[idx] = new KeyedSet("key");
      this.core[idx].add(rule.lookahead);
    }
  }
  LR1Set.prototype.toString = function() {
    var s = "";
    for (var i = 0; i < this.coreRules.length; i++) {
      if (this.noLook[i]) {
        if (s !== "") { s += "\n"; }
        s += this.coreRules[i].toString();
      }
      if (s !== "") { s += "\n"; }
      s += this.coreRules[i].toString() + ", " + this.coreRules[i].toString();
    }
    return "{" + s + "}";
  }
  ...
  SetOfSets.prototype.add = function(item) {
    ...
    if (this.elements[key] === undefined) {
      ...
      return true;
    // } else {
    //   console.log("key = " + key + " and item = " + item.toString() + " and this.elements[key] = " + this.elements[key] + " which would've been set #" + this.ordered.length);
    }
    return false;
  }
  ...
  Grammar.prototype = {
    ...
    initializeParser: function() {
      ...
      console.log("After initialization, this.actions = " + JSON.stringify(this.actions, null, "  "));
    ...
    completeClosure: function(rule_set, inline) {
      var ret = this.closureCache[rule_set.uniqueId];
      if (ret !== undefined) return ret;
      ret = this.closureCache[rule_set.uniqueId] = inline ? rule_set : new KeyedSet(rule_set);
    ...
    // Dragon book, p241
    computeGotoKernel: function(setNum, closedRuleSet, symbol) {
      // console.log("--> Rule_set #" + setNum + " = " + closedRuleSet + ", symbol = " + symbol);
      var ret = new KeyedSet("asString");
      for (var i = 0; i < closedRuleSet.size(); i++) {
        var rule = closedRuleSet.get(i);
        // console.log("    Processing rule " + rule);
        if (rule.position < rule.symbols.length && rule.symbols[rule.position].toString() == symbol) {
          var new_rule = RuleFactory.make(rule.name, rule.symbols, rule.lookahead, rule.position + 1);
      ...
      while (stack.length > 0 && stack[0].todo.length > 0) {
        ...
        if (curr.todo.length > 0) { 
          ...
          if (next.label instanceof Token) {
            ...
            curr.todo.pop();
            curr.done.push(this.applyAction(next.rule, [], next.pos, semActions))
          } else {
    ...
    computeRNTable: function() {
      ...
      for (var i = 0; i < this.states.size(); i++) {
        ...
        for (var j = 0; j < full_state.size(); j++) {
          ...
          // } else if (rule_j.position < rule_j.symbols.length && rule_j.symbols[rule_j.position] instanceof Token) {
          //   var sym = rule_j.symbols[rule_j.position];
          //   console.log("State " + i + " at symbol " + sym + " goes to " + this.rnTable[i][rule_j.symbols[rule_j.position]].push);
          }
        }
      }
    },
    ...
    computeStateKernels: function() {
      ...
      for (var i = 0; i < kernelStates.size(); i++) {
        ...
        for (var j = 0; j < kernelState_i.size(); j++) {
          ...
          // }
          var lookahead = allLookaheads[i][rule_j_no_look.id];
          // console.log("Lookaheads in state " + i + " for rule #" + rule_j_no_look.id + " " + rule_j_no_look.toString(true) + ": " + lookahead);
          if (lookahead) {
            for (var k = 0; k < lookahead.size(); k++)
              state_i.add(RuleFactory.make(rule_j.name, rule_j.symbols, lookahead.get(k), rule_j.position));
          ...
          //   state_i.add(RuleFactory.make(rule_j.name, rule_j.symbols, rule_j.symbols[rule_j.position], rule_j.position));
      ...
      // console.log("This.states = " + this.states.toString(true));
      // console.log("There are " + this.states.size() + " states");
    },

    //////////////////////////////////
    // Algorithm 4.12 (p242) in Dragon book
    applyLookaheads: function(spont, prop, rule_set, set_num, symbol) {
      ...
      // console.log("GOTO(" + rule_set.toString() + ", " + symbol + ") = " + goto_set_num);
      ...
      for (var i = 0; i < rule_set.size(); i++) {
        ...
        if (jPrime === undefined)
          jPrime = this.closureCache[jPrime_rule] = this.completeClosure(new KeyedSet("asString", [jPrime_rule]), true);
        // console.log("Closure(" + jPrime_rule.toString(true) + ") = " + jPrime.toString() + "\n");
        for (var j = 0; j < jPrime.size(); j++) {
          ...
          if (rule_j.position < rule_j.symbols.length && rule_j.symbols[rule_j.position].toString() == symbol) {
            if (rule_j.lookahead !== HASH) {
              ...
              prop[set_num][goto_set_num][rule.id].add(new_rule.id);
            }
          }
        }
      }
