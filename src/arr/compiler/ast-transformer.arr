...
fun make-node-1(typ :: String, id :: A.Expr) -> A.Expr:
  A.s-app(dummy, make-id(typ), [list: id])
end

fun make-list(lst :: List<A.Expr>) -> A.Expr:
  A.s-construct(dummy, A.s-construct-normal, make-id("list"), lst)
end

fun make-str(s :: String) -> A.Expr:
  A.s-str(dummy, s)
end
...
fun visitor-maker(
  ...
  A.s-let(dummy, A.s-bind(dummy, true, make-name(name), A.a-blank), A.s-obj(dummy, method-preamble + methods), false)
end
