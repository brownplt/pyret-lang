...
({
  requires: [
    {
      ...
      protocol: "js-file",
      args: ["ts-codegen-helpers"],
    },
    {
      "import-type": "dependency",
      protocol: "js-file",
      args: ["ts-compile-structs-helpers"],
    },
    {
      "import-type": "dependency",
      protocol: "js-file",
      args: ["provide-serialization"],
    },
  ...
  theModule: function (
    ...
    function freshOrReuseId(
      expr: J.Expression,
      name: string,
    ): [J.Identifier, J.Statement[]] {
    ...
    function freshId(id: A.Name): A.Name {
      ...
      do {
        const baseName =
          id.$name === "s-type-global"
            ? nameToSourceString(id)
            : nameToName(id);
    ...
    function compressRuntimeName(name: string) {
      return name;
    }
    ...
    function rtMethod(name: string, args: Array<J.Expression>) {
      return CallExpression(
        DotExpression(Identifier(RUNTIME), compressRuntimeName(name)),
        args,
      );
    }
      ...
      switch (l.$name) {
        ...
        case "srcloc": {
          const override: A.Srcloc = {
            $name: "srcloc",
            dict: { ...l.dict, source: context.uri },
          };
    ...
    function compileList(
      context: Context,
      exprs: T.List<A.Expr>,
    ): [Array<J.Expression>, Array<J.Statement>] {
      if (exprs.$name === "empty") {
        return [[], []];
      } else {
    ...
    function compileSeq(
      context: Context,
      exprs: T.List<A.Expr>,
    ): [J.Expression, Array<J.Statement>] {
      if (exprs.$name === "empty") {
    ...
    function compileMethodDefinition(
      context: Context,
      method: Variant<A.Member, "s-method-field">,
    ) {
      const args = method.dict.args as Variant<
        T.List<Variant<A.Bind, "s-bind">>,
        "link"
      >;
      ...
      const [methodBody, _] = compileExpr(context, {
        $name: "s-lam",
        dict: lamProps,
      });
      const install = rtMethod("$installMethod", [
        Identifier(self),
        Literal(method.dict.name),
        methodBody,
      ]);
      const body = [
      ...
      return FunctionExpression(
        jsIdOf(constId(`getWrapper_${method.dict.name}`)),
        [],
        BlockStatement(body),
      );
    }
    ...
    function compileObj(
      context: Context,
      expr: Variant<A.Expr, "s-obj">,
    ): [J.Expression, Array<J.Statement>] {
      const fieldsAsArray = listToArray(expr.dict.fields);
      const fieldvs: Array<J.Property> = [],
        stmts = [],
        methods: Array<[string, J.Property]> = [];
      ...
      fieldvs.push(
        Property("$methods", ObjectExpression(methods.map((m) => m[1]))),
      );
      const obj = ObjectExpression(fieldvs);
      const result =
        methods.length === 0 ? obj : rtMethod("$setupMethodGetters", [obj]);
      return [result, stmts];
    }
      ...
      switch (m.$name) {
        case "s-variant-member":
          ...
          switch (m.dict.bind.$name) {
            case "s-tuple-bind":
              throw new InternalCompilerError(m.dict.bind.$name);
            case "s-bind":
    ...
    function compileData(
      context: Context,
      expr: Variant<A.Expr, "s-data-expr">,
    ): [J.Expression, Array<J.Statement>] {
      const [sharedBaseObj, sharedBaseStmts] = compileObj(context, {
        $name: "s-obj",
        dict: { l: expr.dict.l, fields: expr.dict["shared-members"] },
      });
      const sharedBaseName = freshId(constId(`sharedBase_${expr.dict.name}`));
      const variants = listToArray(expr.dict.variants);
      const variantBaseObjs: Array<[A.Name, J.Expression, Array<J.Statement>]> =
        variants.map((v) => {
          const [extensionV, extensionStmts] = compileExpr(context, {
            $name: "s-obj",
            dict: { l: v.dict.l, fields: v.dict["with-members"] },
          });
          ...
          switch (v.$name) {
            case "s-singleton-variant":
              fieldNamesField = Literal(null);
              break;
            case "s-variant": {
              const binds = listToArray(v.dict.members).map((m) => m.dict.bind);
              fieldNamesField = ArrayExpression(
                binds.map((b) => Literal(nameToName(bindToName(b)))),
              );
              break;
            }
          }
          const meta = ObjectExpression([
            dataField,
            nameField,
            Property("$fieldNames", fieldNamesField),
          ]);
          return [
            variantId,
            rtMethod("$createVariant", [
              Identifier(sharedBaseName),
              extensionV,
              meta,
            ]),
            extensionStmts,
      ...
      const variantBaseStmts = variantBaseObjs
        .map((v) => {
      ...
      const variantConstructors = variants.map((v, i) => {
        ...
        switch (v.$name) {
          ...
          case "s-variant":
            ...
            const fields = argList.map((a) =>
              Property(nameToName(a), Identifier(jsIdOf(a))),
            );
            const extension = ObjectExpression(fields);
            const body = BlockStatement([
              ReturnStatement(rtMethod("$makeDataValue", [base, extension])),
            ]);
            const constructor = FunctionExpression(
              jsIdOf(constId(v.dict.name)),
              argList.map(jsIdOf),
              body,
            );
      ...
      function recognizer(name, compareVal, field) {
        ...
        const argIsObject = BinaryExpression(
          "===",
          UnaryExpression("typeof", Identifier(arg)),
          Literal("object"),
        );
        const argIsNotNull = BinaryExpression(
          "!==",
          Identifier(arg),
          Literal(null),
        );
        const argIsVariant = BinaryExpression(
          "===",
          DotExpression(Identifier(arg), field),
          compareVal,
        );
        const check = LogicalExpression(
          "&&",
          LogicalExpression("&&", argIsObject, argIsNotNull),
          argIsVariant,
        );
        const checker = FunctionExpression(
          jsIdOf(constId(isname)),
          [arg],
          BlockStatement([ReturnStatement(check)]),
        );
      ...
      const variantRecognizers = variants.map((v, i) => {
        ...
        return recognizer(
          v.dict.name,
          DotExpression(base, "$variant"),
          "$variant",
        );
      });

      const dataRecognizer = recognizer(
        expr.dict.name,
        Identifier(sharedBaseName),
        "$data",
      );

      const dataPackage = ObjectExpression([
        ...variantConstructors,
        ...variantRecognizers,
        dataRecognizer,
      ]);
    ...
    function compileModule(
      ...
      listToArray(expr.dict["defined-values"]).forEach((dv) => {
        switch (dv.$name) {
          case "s-defined-value": {
            ...
            locs.push(
              ObjectExpression([
                Property("name", Literal(dv.dict.name)),
                Property("srcloc", sloc),
              ]),
            );
            return;
          }
      ...
      const assignAns = AssignmentExpression(
        DotExpression(Identifier(constId("module")), "exports"),
        ans,
      );
      return [
        assignAns,
        [...aStmts, ...context.checkBlockTestCalls, answerVar, ...stmts],
      ];
    }
    ...
    function compileSOp(
      ...
      switch (op) {
        case "op+":
          return rtMethod("_plus", [lv, rv, rtField(NUMBER_ERR_CALLBACKS)]);
          break;
        case "op-":
          return rtMethod("_minus", [lv, rv, rtField(NUMBER_ERR_CALLBACKS)]);
          break;
        case "op*":
          return rtMethod("_times", [lv, rv, rtField(NUMBER_ERR_CALLBACKS)]);
          break;
        case "op/":
          return rtMethod("_divide", [lv, rv, rtField(NUMBER_ERR_CALLBACKS)]);
          break;
        case "op<":
          return rtMethod("_lessthan", [lv, rv, rtField(NUMBER_ERR_CALLBACKS)]);
          break;
        case "op>":
          return rtMethod("_greaterthan", [
            lv,
            rv,
            rtField(NUMBER_ERR_CALLBACKS),
          ]);
          break;
        case "op<=":
          return rtMethod("_lessequal", [
            lv,
            rv,
            rtField(NUMBER_ERR_CALLBACKS),
          ]);
        ...
        case "op>=":
          ...
          break;
        case "op==":
          return CallExpression(rtField(EQUAL_ALWAYS), [lv, rv]);
          break;
        case "op<>":
          return UnaryExpression(
            "!",
            CallExpression(rtField(EQUAL_ALWAYS), [lv, rv]),
          );
          break;
        case "op<=>":
          return CallExpression(rtField(IDENTICAL), [lv, rv]);
          break;
        case "opor":
          return LogicalExpression("||", lv, rv);
          break;
        case "opand":
          return LogicalExpression("&&", lv, rv);
          break;
        case "op^":
          return CallExpression(rv, [lv]);
          break;
        default:
          throw new TODOError(`Not yet implemented: ${op}`);
      }
    }

    function compileOp(
      context: Context,
      expr: Variant<A.Expr, "s-op">,
    ): CompileResult {
      const [lv, lStmts] = compileExpr(context, expr.dict.left);
      const [rv, rStmts] = compileExpr(context, expr.dict.right);
      return [
        compileSOp(context, expr.dict.op, lv, rv),
        [...lStmts, ...rStmts],
      ];
    }
    ...
    function compileIf(
      context: Context,
      branches: (A.IfBranch | A.IfPipeBranch)[],
      compiledElse: CompileResult,
    ): CompileResult {
    ...
    function compileCases(
      context: Context,
      expr: Variant<A.Expr, "s-cases-else">,
    ): CompileResult {
            ...
            const argBinds = listToArray(b.dict.args).map((a, i) => {
              ...
              return Var(
                ...
                BracketExpression(
                  val,
                  BracketExpression(
                    DotExpression(val, "$fieldNames"),
                    Literal(i),
                  ),
                ),
              );
            });
            const assignAnswer = ExpressionStatement(
              AssignmentExpression(Identifier(ans), bodyVal),
            );
            return Case(Literal(b.dict.name), [
              ...argBinds,
              ...bodyStmts,
              assignAnswer,
              BreakStatement,
      ...
      const elseCase = Default([
        ...elseStmts,
        ExpressionStatement(AssignmentExpression(Identifier(ans), elseV)),
      ]);
      return [
        Identifier(ans),
        [
          ...valStmts,
          Var(ans, undefined),
          SwitchStatement(DotExpression(val, "$name"), [
            ...switchBlocks,
            elseCase,
          ]),
        ],
    ...
    function compileSFor(
      context: Context,
      expr: Variant<A.Expr, "s-for">,
    ): CompileResult {
      const binds: A.Bind[] = [];
      const args: A.Expr[] = [];
    ...
    function compileCheckBlock(
      context: Context,
      expr: Variant<A.Expr, "s-check">,
    ): CompileResult {
      const [checkBlockVal, checkBlockStmts] = compileExpr(
        context,
        expr.dict.body,
      );
      ...
      const jsCheckBlockFuncBlock = BlockStatement([
        ...checkBlockStmts,
        ExpressionStatement(checkBlockVal),
      ]);
      const jsCheckBlockFunc = FunctionExpression(
        jsCheckBlockFuncName,
        [],
        jsCheckBlockFuncBlock,
      );
      const blockLoc = Literal(
        formatSrcloc(chooseSrcloc(expr.dict.l, context), true),
      );
      const testerCall = ExpressionStatement(
        rtMethod("$checkBlock", [blockLoc, testBlockName, jsCheckBlockFunc]),
      );
    ...
    function compileCheckTest(
      ...
      function makeCheckExprResult(
        value: T.Either<J.Expression, J.Expression>,
      ) {
        switch (value.$name) {
          case "left":
            return ObjectExpression([
      ...
      switch (op.$name) {
        ...
        case "s-op-is-not": {
          ...
          switch (refinement.$name) {
            case "some":
              const [refinementExpr, refinementStmts] = compileExpr(
                context,
                refinement.dict.value,
              );
              checkOp = {
                $name: "refinement-result",
                refinement: refinementExpr,
                negate,
              };
        ...
        case "s-op-is-roughly": {
          [checkOp, checkOpStmts] = [
            {
              $name: "refinement-result",
              refinement: rtMethod("within", [Literal(0.000001)]),
              negate: false,
            },
            [],
          ];
        ...
        case "s-op-raises-not": {
          ...
          [checkOp, checkOpStmts] = [
            { $name: "expect-raises", negate, predicate: false },
            [],
          ];
          break;
        }
        ...
        case "s-op-raises-violates": {
          ...
          [checkOp, checkOpStmts] = [
            { $name: "expect-raises", negate, predicate: true },
            [],
          ];
          break;
        }
        ...
        case "s-op-satisfies-not": {
          checkOp = {
            ...
            negate: op.$name === "s-op-satisfies-not",
          };
          checkOpStmts = [];
          break;
        }
        default:
          throw new ExhaustiveSwitchError(op);
      }

      function defineBinTest(
        rightExpr: A.Expr,
        binOp: (lhs: J.Expression, rhs: J.Expression) => J.Expression,
      ): CompileResult {
        // Thunk the lhs
        ...
        // LHS exception check
        const lhsException = DotExpression(
          Identifier(lhsParamName),
          "exception",
        );
        const lhsExceptionCheck = exceptionCheck(
          lhsException,
          Identifier(lhsParamName),
          Identifier(rhsParamName),
        );

        const rhsValue = DotExpression(Identifier(rhsParamName), "value");
        // RHS exception check
        const rhsException = DotExpression(
          Identifier(rhsParamName),
          "exception",
        );
        const rhsExceptionCheck = exceptionCheck(
          rhsException,
          Identifier(lhsParamName),
          Identifier(rhsParamName),
        );
        ...
        const successResult = makeCheckOpResult(
          jTestVal,
          Identifier(lhsParamName),
          Identifier(rhsParamName),
        );
        ...
        const testBodyStmts = [
          ...
          rhsExceptionCheck,
          ...checkOpStmts,
          ReturnStatement(successResult),
        ];

        const testBody = BlockStatement(testBodyStmts);
        const testFunc = FunctionExpression(
          compilerName("TEST"),
          [lhsParamName, rhsParamName],
          testBody,
        );

        const testerCallArgs = [lhFunc, rhFunc, testFunc, testLoc];
        const testerCall = ExpressionStatement(
          rtMethod("$checkTest", testerCallArgs),
        );
      ...
      switch (checkOp.$name) {
        case "binop-result": {
          ...
          const right = unwrap(
            rightOpt,
            "Attempting to use a binary check op without the RHS",
          );
        ...
        case "expect-raises": {
          ...
          switch (rightOpt.$name) {
            ...
            case "some": {
              const right = unwrap(
                rightOpt,
                "`raises` checkop did not have a RHS, should be a parsing error",
              );
          ...
          const rhsValue = DotExpression(Identifier(rhsParamName), "value");
          const expectedRhs = makeCheckExprResult({
            $name: "left",
            dict: { v: rhsValue },
          });

          // NOTE(Joe): I don't like what I did here.
          const lhsIsExceptionVal = DotExpression(
            Identifier(lhsParamName),
            "exception",
          );
          ...
          if (rightOpt.$name === "some") {
            ...
            const lhsExceptionVal = DotExpression(
              Identifier(lhsParamName),
              "exception_val",
            );
            const lhsExceptionExtract = rtMethod(TOREPR, [
              rtMethod("$raiseExtract", [lhsExceptionVal]),
            ]);
            let extractionResult: J.Expression;
            if (checkOp.predicate) {
              extractionResult = CallExpression(rhsValue, [lhsExceptionVal]);
            } else {
              extractionResult = CallExpression(
                DotExpression(lhsExceptionExtract, "includes"),
                [rhsValue],
              );
            }
            if (checkOp.negate) {
            ...
            const testResult = LogicalExpression(
              "&&",
              lhsIsExceptionVal,
              extractionResult,
            );
            successResult = makeCheckOpResult(
              testResult,
              Identifier(lhsParamName),
              expectedRhs,
            );
        ...
        case "refinement-result": {
          ...
          const right = unwrap(
            rightOpt,
            "Attempting to use a refinement check without the RHS",
          );
          return defineBinTest(right, (left, right) => {
            if (negate) {
              return UnaryExpression(
                "!",
                CallExpression(refinement, [left, right]),
              );
            } else {
        ...
        case "predicate-result": {
          ...
          const right = unwrap(
            rightOpt,
            "Attempting to use a predicate check without the RHS",
          );
    ...
    function compileLoadTable(
      context: Context,
      expr: Variant<A.Expr, "s-load-table">,
    ): CompileResult {
      ...
      if (spec.length !== 1) {
        ...
        switch (spec[0].$name) {
          ...
          case "s-table-src": {
            ...
            const makeTableFunc = BracketExpression(
              tableId,
              Literal("_makeTableFromTableSkeleton"),
            );
            const changeHeadersFunc = BracketExpression(
              tableId,
              Literal("_tableSkeletonChangeHeaders"),
            );

            const [headersExprArgs, headersExprStmts] = compileExpr(
              context,
              spec[0].dict.src,
            );
            ...
            const changeHeadersExpr = CallExpression(changeHeadersFunc, [
              headersExprArgs,
              headerStrings,
            ]);
    ...
    function compileTableExtend(
      context: Context,
      expr: Variant<A.Expr, "s-table-extend">,
    ): CompileResult {
      ...
      const columnBindsBinds = listToArray(
        expr.dict["column-binds"].dict.binds,
      );
      ...
      listToArray(expr.dict.extensions).forEach((extension) => {
        switch (extension.$name) {
          case "s-table-extend-reducer": {
            ...
            const [reducerExpr, reducerStmts] = compileExpr(
              context,
              extension.dict.reducer,
            );
            ...
            const oneFieldValue = BracketExpression(
              oneFieldValueObj,
              oneFieldValueField,
            );
            ...
            const reduceFieldValue = BracketExpression(
              reduceFieldValueObj,
              reduceFieldValueField,
            );
          ...
          case "s-table-extend-field": {
            ...
            listToArray(expr.dict["column-binds"].dict.binds).forEach(
              (bind) => {
                const bindId = bindToName(bind);

                const getIndexName = freshId(compilerName("columnNumber"));
                const getColumnIndex = BracketExpression(
                  Identifier(TABLE),
                  Literal("_tableGetColumnIndex"),
                );
                const columnIndexArgs = [
                  tableExpr,
                  Literal(nameToName(bindId)),
                ];
                const getIndexRhs = CallExpression(
                  getColumnIndex,
                  columnIndexArgs,
                );
                ...
                const tableRows = BracketExpression(
                  tableExpr,
                  Literal("_rows"),
                );
                const currentRow = BracketExpression(
                  tableRows,
                  Identifier(rowNumberName),
                );
                const indexRhs = BracketExpression(
                  currentRow,
            ...
            const [returnExpr, returnCompiledStmts] = compileExpr(
              context,
            ...
            const reduceFieldValue = FunctionExpression(
              funName,
              funArgs,
              funBody,
            );
    ...
    function compileTableUpdate(
      context: Context,
      expr: Variant<A.Expr, "s-table-update">,
    ): CompileResult {
      // Set the table-import flag
      importFlags["table-import"] = true;
      ...
      const [tableExpr, tableStmts] = compileExpr(
        context,
        expr.dict["column-binds"].dict.table,
      );
      ...
      const columnUpdateZip: Array<[A.Bind, A.Expr]> = listToArray(
        expr.dict["column-binds"].dict.binds,
      ).map((cb, i) => {
        const update = updates[i];
        switch (update.$name) {
          case "s-data-field":
            return [cb, update.dict.value];
          default:
            throw new InternalCompilerError(
              `Invalid update type ${updates[i].$name} at index ${i}`,
            );
        }
      });
      ...
      const appFunc = BracketExpression(
        Identifier(TABLE),
        Literal("_tableTransform"),
      );
      const appArgs = [
        tableExpr,
        ArrayExpression(listColnames),
        ArrayExpression(listUpdates),
      ];
    ...
    function compileTableSelect(
      context: Context,
      expr: Variant<A.Expr, "s-table-select">,
    ): CompileResult {
      // Set the table-import flag
      importFlags["table-import"] = true;

      const func = BracketExpression(
        Identifier(TABLE),
        Literal("_selectColumns"),
      );

      const jsColumns = listToArray(expr.dict.columns).map((c) =>
        Literal(nameToName(c)),
      );
    ...
    function compileTableFilter(
      context: Context,
      expr: Variant<A.Expr, "s-table-filter">,
    ): CompileResult {
      // Set the table-import flag
      importFlags["table-import"] = true;
      ...
      const blockRowElementStmts = listToArray(
        expr.dict["column-binds"].dict.binds,
      ).flatMap((bind) => {
        ...
        const varStmt = Var(
          jsIdOf(bindToName(bind)),
          BracketExpression(Identifier(rowName), Identifier(columnIndexId)),
        );
      ...
      const [predicateExpr, predicateStmts] = compileExpr(
        context,
        expr.dict.predicate,
      );
    ...
    function compileTableOrder(
      context: Context,
      expr: Variant<A.Expr, "s-table-order">,
    ): CompileResult {
      // Set the table-import flag
      importFlags["table-import"] = true;
      ...
      const orderingListElements = listToArray(expr.dict.ordering).map(
        (theOrder) => {
          const theOrderColumn = theOrder.dict.column;
          const theOrderDirection = theOrder.dict.direction;

          const orderColumnField = Property(
            "column",
            Literal(nameToName(theOrderColumn)),
          );
    ...
    function compileTableExtract(
      context: Context,
      expr: Variant<A.Expr, "s-table-extract">,
    ): CompileResult {
      // Set the table-import flag
      importFlags["table-import"] = true;
    ...
    function compileSpy(
      context: Context,
      expr: Variant<A.Expr, "s-spy-block">,
    ): CompileResult {
      ...
      if (context.options.dict["enable-spies"]) {
        ...
        switch (expr.dict.message.$name) {
          case "some": {
            [jsMessageValue, jsMessageStmts] = compileExpr(
              context,
              expr.dict.message.dict.value,
            );
            break;
          }
          case "none":
        ...
        const jsMessageFuncBlock = BlockStatement([
          ...jsMessageStmts,
          jsMessageReturn,
        ]);
        const jsMessageFunc = FunctionExpression(
          jsMessageFuncName,
          [],
          jsMessageFuncBlock,
        );
        ...
        listToArray(expr.dict.contents).forEach((pyretSpyField) => {
          const [jsSpyValue, jsSpyStmts] = compileExpr(
            context,
            pyretSpyField.dict.value,
          );

          const jsSpyExprFuncName = freshId(compilerName("spyExpr"));

          const jsSpyReturn = ReturnStatement(jsSpyValue);
          const jsSpyExprFuncBlock = BlockStatement([
            ...jsSpyStmts,
            jsSpyReturn,
          ]);
          const jsSpyExprFun = FunctionExpression(
            jsSpyExprFuncName,
            [],
            jsSpyExprFuncBlock,
          );
          ...
          const jsSpyLoc = Property(
            "loc",
            Literal(formatSrcloc(pyretSpyField.dict.l, true)),
          );
          const jsSpyExprObj = ObjectExpression([
            jsSpyKey,
            jsSpyExpr,
            jsSpyLoc,
          ]);
    ...
    function compileUpdate(
      ...
      fields.forEach((field) => {
        ...
        stmts.push(
          ExpressionStatement(AssignmentExpression(fieldAccess, fieldValue)),
        );
      });
      return [objId, stmts];
    }
    ...
    function maybeMakeNumberFromString(n: any) {
      if (typeof n === "number") {
        ...
        return rtMethod("_makeNumberFromString", [
          Literal(n.toString()),
          rtField(NUMBER_ERR_CALLBACKS),
        ]);
      }
    ...
    function compileExpr(context: Context, expr: A.Expr): CompileResult {
      switch (expr.$name) {
        ...
        case "s-id": {
          ...
          if (
            runtime.getField(b, "has-key-now").app(key) &&
            !runtime.getField(b, "get-value-now").app(key).dict.origin.dict[
              "new-definition"
            ]
          ) {
            context.freeBindings.set(
              key,
              runtime.getField(b, "get-value-now").app(key),
            );
          }
          ...
          if (isSafe) {
            ...
            return [
              ConditionalExpression(
                ...
                id,
                rtMethod("$messageThrow", [
                  compileSrcloc(context, expr.dict.l),
                  Literal("Uninitialized letrec identifier"),
                ]),
              ),
              [],
            ];
        ...
        case "s-id-modref": {
          const [objv, objStmts] = compileExpr(context, {
            $name: "s-id",
            dict: { l: expr.dict.l, id: expr.dict.id },
          });
        ...
        case "s-lam": {
          ...
          return [
            FunctionExpression(
              jsIdOf(constId(`lam_${expr.dict.name}`)),
              jsArgs,
        ...
        case "s-let-expr": {
          const prelude = [];
          listToArray<A.LetrecBind | A.LetBind>(expr.dict.binds).forEach(
            (v) => {
              const [val, vStmts] = compileExpr(context, v.dict.value);
              switch (v.dict.b.$name) {
                case "s-tuple-bind":
                  throw new InternalCompilerError(
                    "Broken invariant: s-tuple-bind in codegen",
                  );
                case "s-bind":
                  prelude.push(...vStmts);
        ...
        case "s-construct": {
          ...
          const [eltsVals, eltsStmts] = compileList(context, expr.dict.values);
          return [
            CallExpression(DotExpression(cval, "make"), [
              ArrayExpression(eltsVals),
            ]),
            [...cstmts, ...eltsStmts],
          ];
        }
        ...
        case "s-template":
          return [
            rtMethod("throwUnfinishedTemplate", [
              compileSrcloc(context, expr.dict.l),
            ]),
            [],
          ];
        ...
        case "s-if-pipe": {
          ...
          const throwNoMatch = CallExpression(
            rtField("throwNoBranchesMatched"),
            [srcloc, Literal(type)],
          );
          return compileIf(
            context,
        ...
        case "s-assign": {
          ...
          const assnStmt = AssignmentExpression(
            Identifier(jsIdOf(expr.dict.id)),
            rhs,
          );
        ...
        case "s-bracket": {
          ...
          const bracketExpr = CallExpression(DotExpression(lhs, "get-value"), [
            key,
          ]);
        ...
        case "s-extend": {
          ...
          return [
            rtMethod("$extend", [objV, extensionV]),
            [...objStmts, ...extensionStmts],
          ];
        }
        case "s-for":
          return compileSFor(context, expr);
        case "s-tuple": {
        ...
        case "s-check-expr":
          return compileExpr(context, expr.dict.expr);
        case "s-check":
          return compileCheckBlock(context, expr);
        case "s-check-test":
          return compileCheckTest(context, expr);

        case "s-table":
          return compileTable(context, expr);
        case "s-load-table":
          return compileLoadTable(context, expr);
        case "s-table-extend":
          return compileTableExtend(context, expr);
        case "s-table-update":
          return compileTableUpdate(context, expr);
        case "s-table-filter":
          return compileTableFilter(context, expr);
        case "s-table-select":
          return compileTableSelect(context, expr);
        case "s-table-order":
          return compileTableOrder(context, expr);
        case "s-table-extract":
          return compileTableExtract(context, expr);

        case "s-spy-block":
          return compileSpy(context, expr);
        case "s-update":
          return compileUpdate(context, expr);
    ...
    function importToDep(i: A.ImportType): CS.Dependency {
      switch (i.$name) {
        case "s-const-import":
          return { $name: "builtin", dict: { modname: i.dict.mod } };
        case "s-special-import":
          return {
            $name: "dependency",
            dict: { protocol: i.dict.kind, arguments: i.dict.args },
          };
      }
    }
    ...
    function depToKey(d: CS.Dependency): string {
      switch (d.$name) {
        case "dependency":
          return `${d.dict.protocol}(${listToArray(d.dict.arguments).join(", ")})`;
        case "builtin":
          return `builtin(${d.dict.modname})`;
      }
    }
    ...
    function createPrelude(
      ...
      function getBaseDir(source: string, buildDir: string): [string, string] {
        ...
        for (let i = 0; i < shorter; i += 1) {
          if (sourcePath[i] !== buildDir[i]) {
            cutoffIndex = i;
            break;
          }
        }
        if (cutoffIndex === 0) {
          cutoffIndex = buildDir.length;
        }
      ...
      function getCompiledRelativePath(baseDir: string, source: string) {
        ...
        let projectRelativePath = P.dirname(
          source.substring(baseDir.length + 1),
        );
        if (projectRelativePath === ".") {
          return "./";
        }
        let parentPath = projectRelativePath
          .split("/")
          .map((v) => "../")
          .join("");
        return parentPath;
      ...
      const [baseDir, absoluteSource] = getBaseDir(
        provides.dict["from-uri"],
        options.dict["base-dir"],
      );
      ...
      const runtimeBuiltinRelativePath: A.Option<string> =
        options.dict["runtime-builtin-relative-path"];
      ...
      function uriToImport(uri: string, name: A.Name): Array<J.Statement> {
        if (uri.startsWith("builtin://")) {
          ...
          return [
            Var(
              jsIdOf(name),
              CallExpression(Identifier(constId("require")), [
                Literal(thePath),
              ]),
            ),
            ExpressionStatement(
              rtMethod("addModule", [Literal(uri), Identifier(jsIdOf(name))]),
            ),
          ];
      ...
      function importBuiltin(bindName: A.Name, name: string) {
        ...
        switch (runtimeBuiltinRelativePath.$name) {
          case "some":
            thePath = runtimeBuiltinRelativePath.dict.value + name;
            break;
          case "none":
            thePath = relativePath + "../builtin/" + name;
            break;
        }
        return Var(
          bindName,
          CallExpression(Identifier(constId("require")), [Literal(thePath)]),
        );
      }
      ...
      const manualImports = [];
      if (importFlags["table-import"]) {
        manualImports.push(tableImport);
      }
      ...
      const explicitImports = listToArray(imports)
        .map((importStmt) => {
          switch (importStmt.$name) {
            case "s-import":
              ...
              let uri = runtime
                .getField(env.dict["my-modules"], "get-value")
                .app(depKey);
              switch (options.dict["compile-mode"].$name) {
                ...
                case "cm-builtin-stage-1":
                case "cm-builtin-general":
                  uri = "builtin://" + P.basename(uri, ".arr");
                  break;
              }
      ...
      function envUriByValueNameValue(env, name) {
        ...
        // This should use a clean CompileEnv interface method
        return runtime
          .getField(env.dict.globals.dict.values, "get-value")
          .app(name).dict["uri-of-definition"];
      }
      ...
      */
      ...
      const fromModules = [...freeBindings.values()].map((binding) => {
        ...
        return Var(
          jsIdOf(binding.dict["atom"]),
          rtMethod("getModuleValue", [Literal(uri), Literal(name)]),
        );
      });
      ...
      const setupRuntime = [
        runtimeImport,
        ExpressionStatement(
          rtMethod("$claimMainIfLoadedFirst", [
            Literal(provides.dict["from-uri"]),
          ]),
        ),
        ExpressionStatement(
          rtMethod("$clearTraces", [Literal(provides.dict["from-uri"])]),
        ),
        ExpressionStatement(
          rtMethod("$clearChecks", [Literal(provides.dict["from-uri"])]),
        ),
      ];
     ...
     */
    function deepCheck<T extends { $name: string; dict: {} }>(
      visitor: Partial<Record<T["$name"], any>>,
      val: T,
    ) {
      ...
      if (checkerName in A.dict.values.dict) {
        if (!A.dict.values.dict[checkerName].app(val)) {
          throw new InternalCompilerError(
            `Value didn't satisfy ${checkerName}: ${val}`,
          );
        }
      }
    ...
    function assertMapIsDoingItsJob(prog: A.Program) {
      ...
      if (prog === after) {
        throw new InternalCompilerError(
          "AST map visitor returned an identical object",
        );
      }
      deepCheck({}, after);
      return after;
    ...
    function serializeFileRequires(
      name: string,
      uriKey: string,
      protocol: string,
      options,
      ...
      switch (mode.$name) {
        ...
        case "cm-builtin-stage-1":
          return serializeBuiltinRequires(name, options);
        case "cm-builtin-general":
          return serializeBuiltinRequires(name, options);
        default:
          throw new ExhaustiveSwitchError(mode);
    ...
    function serializeRequires(
      ...
      Object.keys(env.dict["all-modules"].$underlyingDict).forEach(
        (uriKey: string) => {
    ...
    function compileProgram(
      ...
      return {
        requires: escodegen.generate(
          ArrayExpression(serializeRequires(env, options)),
          jsonOptions,
        ),
