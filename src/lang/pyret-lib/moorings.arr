...
fun interp-state():
  ...
  
  {
   ...
   Store: Store,
   mt-store:mt-store, is-mt-store:is-mt-store,
   a-store:a-store, is-a-store:is-a-store,

   Result: Result,
   result:result, is-result:is-result,
   ...
   lam:lam, is-lam:is-lam,
   app:app, is-app:is-app,
   assign:assign, is-assign:is-assign,
   do:do, is-do:is-do,
...
fun interp-records():
  ...
  data Expr:
    ...
    | record(fields :: List<FieldExpr>)
    | lookup(rec :: Expr, field-name :: String)
    | extend(rec :: Expr, field-name :: String, new-value :: Expr)
  end
  ...
  fun parse(prog) -> Expr:
    fun convert-field(sexpr):
      field(sexpr.get(0), convert(sexpr.get(1)))
    end
    fun convert(sexpr):
      if List(sexpr):
        head = sexpr.first
        if head == "string":
          ...
          record(map(convert-field, sexpr.rest))
        else if head == "lookup":
          lookup(convert(sexpr.get(1)), sexpr.get(2))
        else if head == "extend":
          extend(convert(sexpr.get(1)),
                 sexpr.get(2),
                 convert(sexpr.get(3)))
        else if head == "assign":
          assign(sexpr.get(1), convert(sexpr.get(2)))
...

  {
   ...
   strV: strV, is-strV: is-strV,
   funV: funV, is-funV: is-funV,
   recV: recV, is-recV: is-recV,

   FieldV: FieldV,
   fieldV: fieldV, is-fieldV: is-fieldV,
   ...
   assign:assign, is-assign:is-assign,
   do:do, is-do:is-do,
   record:record, is-record:is-record,
   lookup:lookup, is-lookup:is-lookup,
   extend:extend, is-extend:is-extend,

   Field : Field,
   field:field, is-field:is-field,
...
fun interp-with():
  ...
  data Env:
    | mt-env
    | an-env(name :: String, loc :: Number, env :: Env)
  end
  
  data Store:
    | mt-store
    | a-store(loc :: Number, val :: Value, store :: Store)
  end
    ...
    fun convert(sexpr):
      if List(sexpr):
        ...
        if head == "string":
          ...
          with(convert(sexpr.get(1)), convert(sexpr.get(2)))
        else if head == "assign":
          assign(sexpr.get(1), convert(sexpr.get(2)))
        else if head == "do":
          do(map(convert, sexpr.rest))
...
fun interp-lifting-locals():
  ...
  data Expr:
    ...
    | bop(op :: Operator, left :: Expr, right :: Expr)
    | cif(cond :: Expr, consq :: Expr, altern :: Expr)
    | lam(params :: List<String>, body :: Block)
    | app(func :: Expr, args :: List<Expr>)
  ...
  fun parse(prog) -> Block:
    ...
    fun convert-block(sexpr :: List):
      stmts = map(convert-stmt, sexpr)
      cases(Stmt) stmts.last():
        | expr-stmt(e) => block(stmts.take(stmts.length() - 1), e)
        | _ => raise("The last statement in a block must be an expression. Found "
                     + tostring(stmts.last()))
      end
    end
    ...
    fun convert(sexpr):
      if List(sexpr):
        ...
        if head == "string":
          ...
          lam(sexpr.get(1), convert-block(sexpr.rest.rest))
        else if head == "+":
          bop(plus, convert(sexpr.get(1)), convert(sexpr.get(2)))
        else if head == "-":
          bop(minus, convert(sexpr.get(1)), convert(sexpr.get(2)))
...

  {
   ...
   Block: Block,
   block:block, is-block:is-block,

   Stmt: Stmt,
   defvar:defvar, is-defvar:is-defvar,
   assign:assign, is-assign:is-assign,
   expr-stmt:expr-stmt, is-expr-stmt:is-expr-stmt,
...
fun calculate-locals-typed-with():
  ...
  data Expr:
    ...
    | with(namespace :: Expr, type :: Type, body :: Expr)
  ...
  fun parse(prog) -> Expr:
    ...
    fun convert-type(sexpr):
      if List(sexpr):
        head = sexpr.first
        if head == "->":
          funT(map(convert-type, sexpr.get(1)),
  ...
  
  {
   ...
   Type:Type,
   numT:numT, is-numT:is-numT,
   funT:funT, is-funT:is-funT,
   recordT:recordT, is-recordT:is-recordT,

   FieldT:FieldT,
   fieldT:fieldT, is-fieldT:is-fieldT,
...
cs173 = {
  ...
  interp-records: interp-records(),
  interp-with: interp-with(),
  interp-lifting-locals: interp-lifting-locals(),
  calculate-locals: calculate-locals(),
  calculate-locals-with:calculate-locals-with(),
  calculate-locals-typed-with:calculate-locals-typed-with()
}
