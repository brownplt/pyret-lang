  ...
  cases (Term) e:
    ...
    | else =>
      cases (Pattern) p:
        ...
        | p-tag(lhs, rhs, body) =>
          panic("Encountered a p-tag while matching, but it should only be used internally.")
    ...
    is left({environment(
        ...
        [string-dict: "l1", [list:
            ...
            environment(
              ...
              [string-dict: ])]]);
      p-surf("hello", [list:
          p-option(some(p-pvar("j", none))),
          p-list(seq-ellipsis-list(
              [list:
                p-list(seq-cons(p-pvar("a", none), seq-cons(p-pvar("b", none), seq-empty))),
                p-list(seq-cons(p-pvar("a", none), seq-cons(p-pvar("b", none), seq-empty)))],
              "l1"))
        ])
    })
