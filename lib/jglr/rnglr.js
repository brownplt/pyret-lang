define("jglr/rnglr", ["jglr/cyclicJSON"], function(cycle) {
  ...
  Atom.fromSerializable = function(obj) {
    ...
    if (typeof obj === "number") {
      if (obj > 0) return Nonterm.byIndex[obj];
      return Token.byIndex[-obj];
    }
  ...
  function Nonterm(name) {
    this.name = name;
    this.key = "@" + this.name;
    if (!Nonterm.byName[name]) {
      Nonterm.byName[name] = Nonterm.byIndex.length;
      Nonterm.byIndex.push(name);
    }
  }
  Nonterm.byName = {}
  Nonterm.byIndex = [undefined]
  Nonterm.prototype = Object.create(Atom.prototype);
  Nonterm.prototype.toSerializable = function() {
    return Nonterm.byName[this.name];
  }
  ...
  function Token(name, value) {
    ...
    if (!Token.byName[name]) {
      Token.byName[name] = 0 - Token.byIndex.length;
      Token.byIndex.push(name);
    }
  }
  Token.byName = {}
  Token.byIndex = [undefined]
  ...
  ReduceAction.prototype.toString = function(hideRule) { 
    if (hideRule)
      ...
      return "Reduce(" + this.rule.id + ":" + this.rule.toString() + ", m" + this.rule.position + ", f" + this.f + ")";
  }
  ...
  Rule.ListSnoc = function(tl, hd, shouldInline) {
    ...
    var serial = [Nonterm.byName[tl], Nonterm.byName[hd], shouldInline, "LS"];
    ret.toSerializable = function() { return serial; }
    if (shouldInline) {
  ...
  Rule.Inline.toSerializable = function() { return "I"; }
  Rule.I = Rule.Inline;
  ...
  Rule.KeepOnly = function(names, shouldInline) {
    ...
    var serial = [];
    for (var i = 0; i < names.length; i++) {
      serial.push(Nonterm.byName[names[i]]);
    }
    serial.push(shouldInline);
    serial.push("KO");
    ret.toSerializable = function() { return serial; }
    if (shouldInline) {
  ...
  Rule.fromSerializable = function(rulesByOldId, id) {
    ...
    if (obj[3]/*.hasOwnProperty("like")*/) {
      var base = Rule.fromSerializable(rulesByOldId, obj[3]/*.like*/);
      var lookahead = obj[0]/*.lookahead*/;
      var pos = obj[1]/*.position*/;
      return RuleFactory.make(base.name, base.symbols, Atom.fromSerializable(lookahead), pos);
    } else {
      var sym = [];
      if (obj[1]/*symbols*/) {
        for (var i = 0; i < obj[1]/*.symbols*/.length; i++) {
          sym[i] = Atom.fromSerializable(obj[1]/*.symbols*/[i]);
        }
      }
      return RuleFactory.make(Nonterm.byIndex[obj[0]].name/*.name*/,
                              sym,
                              // lookahead never seems to happen
                              undefined/*Atom.fromSerializable(obj.lookahead)*/,
                              0 + obj[2]/*.position*/);
    }
  }
  ...
  Rule.prototype = {
    ...
    toSerializable: function() {
      ...
      if (this.lookahead) {
        ...
        return [
          this.lookahead.toSerializable(),
          this.position,
          undefined,
          this.withLookaheadPosition(undefined, 0).id
        ];
      }
      ret[0]/*.name*/ = Nonterm.byName[this.name];
      if (this.symbols.length > 0) {
        ret[1]/*.symbols*/ = [];
        for (var i = 0; i < this.symbols.length; i++) {
          ret[1]/*.symbols*/[i] = this.symbols[i].toSerializable();
        }
      }
      if (this.position != 0) {
        ret[2]/*.position*/ = this.position;
      }
      return ret;
    }
  }
  ...
  Grammar.fromSerializable = function(obj) {
    ...
    Link.NextLinkId = 0;
    SPPFNode.NextId = 0
    // skip index 0; it's bogus
    for (var i = 1; i < obj.nonterms.length; i++) {
      Nonterm.byIndex[i] = new Nonterm(obj.nonterms[i]);
    }
    // skip index 0; it's bogus
    for (var i = 1; i < obj.tokens.length; i++) {
      Token.byIndex[i] = new Token(obj.tokens[i]);
    }
    for (var id in obj.rulesByOldId) {
      rulesByOldId[id] = Rule.fromSerializable(obj.rulesByOldId, id);
      var actionByOldId = obj.actionsByOldId[id];
      if (typeof actionByOldId === "string") {
        actionByOldId = Rule[actionByOldId];
      } else if (actionByOldId instanceof Array) {
        var kind = actionByOldId.pop();
        for (var i = 0; i < actionByOldId.length; i++) {
          if (typeof actionByOldId[i] === "number")
            actionByOldId[i] = obj.nonterms[actionByOldId[i]];
        }
        actionByOldId = Rule[kind].apply(null, actionByOldId);
      }
      actionsByOldId[id] = actionByOldId;
    ...
    for (var i = 0; i < obj.rnTable.length; i++) {
      ...
      for (var j = 0; j < g.atoms.size(); j++) {
        ...
        var atomStr = atom.toString();
        newRow[atomStr] = {push: undefined, reductions: new KeyedSet("key")};
        if (atomStr in tableRow) {
          if (tableRow[atomStr][0]/*.reductions*/) {
            for (var k = 0; k < tableRow[atomStr][0]/*.reductions*/.length; k++) {
              var red = tableRow[atomStr][0]/*.reductions*/[k];
              newRow[atomStr].reductions.add(new ReduceAction(rulesByOldId[red[0]], red[1]));
            }
          }
          if (tableRow[atomStr][1]/*.push*/) {
            //assert.equal(newRow[atom].push, undefined, "Already have a push action for atom " + atom);
            newRow[atomStr].push = tableRow[atomStr][1]/*.push*/;
          }
          if (tableRow[atomStr][2]/*.accept*/)
            newRow[atomStr].accept = true;
        }
      }
    }
    g.acceptStates = [];
    ...
    */
    ...
    for (var i = 0; i < obj.eSPPFs.length; i++) {
      var label = obj.eSPPFs[i][0]/*.label*/;
      if (label === "EPSILON")
        label = EPSILON;
    ...
    for (var i = 0; i < obj.eSPPFs.length; i++) {
      ...
      if (src[1]/*.kids*/) {
        var kids = [];
        for (var j = 0; j < src[1]/*.kids*/.length; j++)
          kids.push(g.eSPPFs[src[1]/*.kids*/[j]].null);
        dest.addChildren(rulesByOldId[src[2]/*.rule*/], kids, this);
      }
    }
    return g;
  }
      ...
      */
      ...
      for (var name in this.rules) {
        for (var i = 0; i < this.rules[name].length; i++) {
          ...
          ret.actionsByOldId[rule_noLookahead.id] = this.getAction(rule_noLookahead).toSerializable();
          ...
          if (tableRow[name].push === undefined && !tableRow[name].accept && tableRow[name].reductions.size() === 0) {
            ...
            if (entry.reductions.size() > 0) {
              ...
              for (var j = 0; j < entry.reductions.size(); j++) {
                ...
                dest[0]/*.reductions*/.push([red.rule.id, red.f]);
            ...
            if (entry.push !== undefined) {
              dest[1]/*.push*/ = entry.push;
            }
            if (entry.accept) dest[2]/*.accept*/ = true;
          }
        }
      }
      ret.I = this.I;
      ...
      for (var i = 0; i < this.eSPPFs.length; i++) {
        ...
        if (null_eSPPF.kids) {
          ...
          if (null_eSPPF.rule)
            ret.eSPPFs[i] = [label, kids, null_eSPPF.rule.id];//{label: label, kids: kids, rule: null_eSPPF.rule.id};
          else
            ret.eSPPFs[i] = [label, kids]; //{label: label, kids: kids};
        } else {
          if (null_eSPPF.rule)
            ret.eSPPFs[i] = [label, undefined, null_eSPPF.rule.id];//{label: label, rule: null_eSPPF.rule.id};
          else
            ret.eSPPFs[i] = [label];//{label: label};
        }
      }
      ret.nonterms = Nonterm.byIndex;
      ret.tokens = Token.byIndex;
      return ret;
    },
    ...
    */
    ...
    getEpsilonSPPF: function(i, pos) {
      function clone(node, cache) {
        var ret = cache[node.sppfId];
        if (ret !== undefined) return ret;
        ret = cache[node.sppfId] = ret;
