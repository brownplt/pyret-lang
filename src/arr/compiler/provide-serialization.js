({
    ...
    theModule: function (runtime, _, __, TCH, escodegen, P) {
        const { ExhaustiveSwitchError, Literal, ObjectExpression, Property, compileSrcloc, listToArray, ArrayExpression, nameToKey, InternalCompilerError, nameToName, ConditionalExpression, } = TCH;
        ...
        function builtinSrcloc(uri) {
            return {
                $name: 'builtin',
                dict: { "module-name": uri },
            };
        }
        function compileOrigin(bo) {
        ...
        function compileTypeMember(name, typ) {
            return Property(name, compileProvidedType(typ));
        }
        function compileTypeVariant(variant) {
            switch (variant.$name) {
                case 't-variant': {
        ...
        function compileProvidedData(de) {
            switch (de.$name) {
                case 'd-alias': {
        ...
        function compileProvidesOverrideUri(provides, override) {
        ...
        function compileProvidedType(typ) {
            switch (typ.$name) {
                case 't-name': {
                    switch (typ.dict['module-name'].$name) {
                        case 'local': {
                            return ObjectExpression([
                                ...
                                Property("name", Literal(nameToName(typ.dict.id)))
                            ]); // TODO: toname or key?
                        }
                        case 'module-uri': {
                            return ObjectExpression([
                ...
                case 't-arrow': {
                    return ArrayExpression([Literal("arrow"),
                        ArrayExpression(listToArray(typ.dict.args).map(compileProvidedType)), compileProvidedType(typ.dict.ret)]);
                }
                case 't-app': {
                    return ArrayExpression([Literal("tyapp"), compileProvidedType(typ.dict.onto),
                ...
                case 't-record': {
                    return ArrayExpression([Literal("record"), ObjectExpression(typ.dict.fields.$underlyingMap.keys().map((key) => {
                            return compileTypeMember(key, typ.dict.fields.$underlyingMap.get(key, null));
                        }))]);
                }
                case 't-tuple': {
                    return ArrayExpression([Literal("tuple"), ArrayExpression(listToArray(typ.dict.elts).map(compileProvidedType))]);
                }
                case 't-forall': {
                    return ArrayExpression([Literal("forall"),
                ...
                case 't-data-refinement': {
                    return ArrayExpression([Literal("data%"), compileProvidedType(typ.dict['data-type']), Literal(typ.dict['variant-name'])]);
                }
                default:
                    return ConditionalExpression(Literal(false), Literal(String(typ)), Literal("tany"));
            }
        }
        function compileProvides(provides) {
                            ...
                            case 'v-fun': {
                                ...
                                switch (val.dict.flatness.$name) {
                                    case 'none':
                                        ...
                                        break;
                                    case 'some':
                                        flatness = val.dict.flatness.dict.value;
                                        break;
                                }
                                return Property(v, ObjectExpression([
