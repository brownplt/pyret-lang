...
(define (ann-check loc ann)
  ...
  (define (code-wrapper s args result type get-fun initial-check)
    ...
    (define wrapargs (map (lambda (a) (s-bind s #f (gensym "arg") a)) args))
    (define (check-arg bind)
      (match bind
        [(s-bind s shadow id ann) (wrap-ann-check s ann (s-id s id))]))
    (mk-lam s (list (s-bind s #f funname ann)) ann
     (mk-contract-doc ann)
     (s-block s
      (list
       (s-app loc
              ...
              (list initial-check
                    (s-id loc funname)
                    ann-str))
       (s-let s (s-bind s #f gotten-funname (a-blank)) (get-fun (s-id s funname)))
       (s-extend
         s
  ...
  (define (record-wrapper s fields)
    ...
    (mk-lam s (list (s-bind s #f recname (a-blank))) ann
            (mk-contract-doc ann)
  ...
  (define (mk-flat-checker checker)
    ...
    (mk-lam loc (list (s-bind loc #f argname (a-blank))) ann
            (mk-contract-doc ann)
            (s-app
             loc
             (s-id loc 'check-brand)
  ...
  (match ann
    ...
    [(a-blank)
     ...
     (mk-lam loc (list (s-bind loc #f argname (a-blank))) (a-blank)
             (mk-contract-doc ann)
             (s-id loc argname)))]
    [(a-any)
     (let ([argname (gensym "any-arg")])
    ...
    [(a-pred s ann pred)
     ...
     (mk-lam loc (list (s-bind loc #f argname (a-blank))) (a-blank)
             ...
             (s-block s
               (list
                 (s-var s (s-bind s #f tempname (a-blank))
                          (s-app loc
                                 ann-wrapper
                                 (list (s-id loc argname))))
                 (s-var s (s-bind s #f result (a-blank))
                          (s-app loc
                                 pred
...
(define (cc-env ast env)
       ...
       (cond
        ...
        [else
         ...
         (define field-block
          (s-block s
            (list
              (s-let s (s-bind s #f val-id (a-blank)) (cc-env value env))
              (s-if-else s
  ...
  (match ast
    ...
    [(s-lam s typarams args ann doc body check)
     (define (new-arg b)
      (match b
        [(s-bind s shadow id ann) (s-bind s #f (gensym id) (a-blank))]))
