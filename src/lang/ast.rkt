...
(struct s-bind s-ast (syntax id ann shadow) #:transparent)
...
(define (subst expr1 sub-id expr2)
  ...
  (match expr1
    ...
    [(s-bind s id ann shadow) (s-bind s id (subst-ann ann sub-id expr2) shadow)]
  ...
  (define (variant-ids variant)
    (match variant
      ...
      [(s-datatype-singleton-variant _ name _)
       (list name (make-checker-name name))]))
  (match expr
    [(s-var _ (s-bind _ x _ _) _) (list x)]
    [(s-let _ (s-bind _ x _ _) _) (list x)]
...
(define (free-ids expr)
  ...
  (define (free-ids-bind b)
    (match b
      [(s-bind _ id ann _) (free-ids-ann ann)]))
  ...
  (match expr
    ...
    [(s-try _ body (s-bind _ id _ _) except)
...
(define (equiv-ast ast1 ast2)
  ...
  (define (equiv-ast-bind b1 b2)
    (match (cons b1 b2)
      [(cons
        (s-bind _ id1 ann1 shadow1)
        (s-bind _ id2 ann2 shadow2))
       (and
        (equal? id1 id2)
        (equiv-ast-ann ann1 ann2)
  ...
  (define result
    (match (cons ast1 ast2)
      ...
      [(cons (s-bind _ id1 ann1 shadow1) (s-bind _ id2 ann2 shadow2))
...
(define (replace-typarams-binds typarams)
  (lambda (bind)
    (match bind
      [(s-bind s id ann shadow)
       (s-bind s id ((replace-typarams typarams) ann) shadow)]
      [_ bind])))
