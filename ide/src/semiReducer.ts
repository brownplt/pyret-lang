...
import {
  Action, ActionType, isActionType, ActionOfType,
} from './action';
...
export type SemiReducer<K extends ActionType> = (
  state: State, action: ActionOfType<K>
) => PartialState;
...
// Applies the first stateUpdate with a .state field that matches state.compileState to
...
export function guard<K extends ActionType>(
  actionType: K,
  semiReducer: SemiReducer<K>,
): SemiReducer<ActionType> {
...
export function guardUpdates<K extends ActionType>(
  actionType: K,
  stateUpdates: StateUpdates<K> | (() => StateUpdates<K>),
): SemiReducer<ActionType> {
...
export function combineSemiReducers(semiReducers: Array<SemiReducer<ActionType>>): Reducer {
  return (nextState, action) => semiReducers.reduce(
    (state, r) => ({ ...state, ...r(state, action) }),
    nextState,
  );
}
