...
#   L.shuffle(range(0, 500).map(num-sqrt)))
# rendered = VC.render-chart(va-hist).background-color(C.gray)
#   .x-axis("This way!").y-axis("That way!").height(600)
# N = 500
# vals = range(0, N)
...
rendered = VC.render-chart(va-hist)
  .background-color(C.gray)
  .x-axis("This way!")
  .y-axis("That way!")
