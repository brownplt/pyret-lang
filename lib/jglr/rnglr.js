define(["./cyclicJSON"], function(cycle) {
  ...
  function Rule(name, symbols, lookahead, position, action, flags) {
  ...
  Rule.defaultASTToString = function(show_pos) {
    ...
    if (this.flags !== undefined)
      toStr += " " + JSON.stringify(this.flags);
    toStr += ")";
    return toStr; 
  }
  ...
  RuleFactory.cache = {};
  RuleFactory.byId = {};
  RuleFactory.make = function(name, symbols, lookahead, pos, action, flags) {
    if (action === undefined)
      action = Rule.defaultAction;
    pos = pos || 0;
    var stringFlags = "undefined";
    if (flags !== undefined)
      stringFlags = "" + JSON.stringify(flags);
      ...
      if (findBySymbols) {
        ...
        if (findByLookahead) {
          ...
          if (findByPos) {
            ...
            if (findByAction) {
              var findByFlags = findByAction[stringFlags];
              if (findByFlags) return findByFlags;
              else {
                var rule = new Rule(name, symbols, lookahead, pos, action, flags);
                RuleFactory.byId[rule.id] = rule;
                findByAction[stringFlags] = rule;
                return rule;
              }
            } else {
              var rule = new Rule(name, symbols, lookahead, pos, action, flags);
              RuleFactory.byId[rule.id] = rule;
              var byFlags = {}; byFlags[stringFlags] = rule;
              findByPos[action] = byFlags;
              return rule;
            }
          } else {
            var rule = new Rule(name, symbols, lookahead, pos, action, flags);
            RuleFactory.byId[rule.id] = rule;
            var byFlags = {}; byFlags[stringFlags] = rule;
            var byAction = {}; byAction[action] = byFlags;
  ...
  Rule.fromSerializable = function(rulesByOldId, id) {
    ...
    if (obj.hasOwnProperty("like")) {
      ...
      return RuleFactory.make(base.name, base.symbols, Atom.fromSerializable(lookahead), base.position, base.action, base.flags);
    } else {
      var sym = [];
      for (var i = 0; i < obj.symbols.length; i++) {
        sym[i] = Atom.fromSerializable(obj.symbols[i]);
      }
      return RuleFactory.make(obj.name, sym, Atom.fromSerializable(obj.lookahead), obj.position, eval(obj.action), obj.flags && obj.flags !== "undefined" ? JSON.parse(obj.flags) : undefined);
    }
  }
  ...
  Rule.equals = function(thiz, that) {
    ...
    if (thiz.flags === undefined && that.flags === undefined) return true;
    if (thiz.flags !== undefined || that.flags === undefined) return false;
    for (var flag in thiz.flags)
      if (thiz.hasOwnProperty(flag))
        if (thiz.flags[flag] !== that.flags[flag]) return false;
    for (var flag in that.flags)
      if (that.hasOwnProperty(flag))
        if (thiz.flags[flag] !== that.flags[flag]) return false;
    return true;
  }
  ...
  Rule.prototype = {
    ...
    withLookahead: function(lookahead) { 
      return RuleFactory.make(this.name, this.symbols, lookahead, this.position, this.action, this.flags); 
    },
    withFlags: function(flags) {
      return RuleFactory.make(this.name, this.symbols, this.lookahead, this.position, this.action, flags);
    },
    toSerializable: function() {
      ...
      if (this.flags) {
        ret.flags = JSON.stringify(this.flags);
      }
  ...
  SPPFNode.prototype.addChildren = function(rule, kids, sortingFlag) {
    if (this.kids === undefined && this.ambig === undefined) {
      ...
      if (sortingFlag === undefined || rule.flags === undefined || rule.flags[sortingFlag] === undefined) {
        this.ambig.push({kids: kids, rule: rule});
      } else {
        var newFlag = rule.flags[sortingFlag];
        for (var i = 0; i < this.ambig.length; i++) {
          var flags = this.ambig[i].rule.flags;
  ...
  Grammar.prototype = {
    ...
    addRule: function(name, symbols, action, flags) {
      ...
      if (name instanceof Rule) {
        ...
        new_rule = RuleFactory.make(name, symbols, undefined, undefined, action, flags);
      }
      ...
      for (var i = 0; i < rule_set.size(); i++) {
        ...
        if (rule.position < rule.symbols.length) {
          if (rule.symbols[rule.position].toString() == symbol) {
            var new_rule = RuleFactory.make(rule.name, rule.symbols, undefined, rule.position + 1, rule.action, rule.flags);
            ...
            for (var idx = 0; idx < ntFirst.size(); idx++) {
              ...
              for (var j = 0; j < rules_a.length; j++) {
                ...
                if (rule_a.symbols.length > 0 && rule_a.symbols[0].toString() == symbol) {
                  ...
                  var new_rule = RuleFactory.make(rule_a.name, rule_a.symbols, undefined, 1, rule_a.action, rule_a.flags);
                  ret.add(new_rule);
                }
              }
            }
    ...
    addChildren: function(rule, y, kids, f, pos) {
      ...
      if (!found)
        y.addChildren(rule, lambda, this.ambiguitySortingFlag);
    },
      ...
      for (var name in this.rules) {
        ...
        for (var i = 0; i < rules_name.length; i++) {
          ...
          if (this.computeFirstOfStrings(rule.symbols).contains(EPSILON)) {
            ...
            name_eSPPF.addChildren(rule, kids, this.ambiguitySortingFlag);
          }
        }
      }
    ...
    computeRNTable: function() {
      ...
      for (var i = 0; i < this.states.size(); i++) {
        ...
        // console.log("FullState " + i + " = " + state_i.toString());
        for (var j = 0; j < full_state.size(); j++) {
          ...
          if (rule_j.name === this.start && rule_j.position === rule_j.symbols.length && rule_j.lookahead === EOF) {
            ...
            this.rnTable[i][EOF].accept = true;
                     ...
                     this.computeFollowAtPosition(rule_j.withLookahead(undefined).withFlags(undefined), 
                                                  rule_j.position).contains(EPSILON)) {
            // console.log("i = " + i + " rule_j = " + rule_j + " and its follow at position " + rule_j.position + " contains EPSILON");
            if (this.rnTable[i][rule_j.lookahead] === undefined) {
              throw("Can't get rnTable actions for i = " + i + " and symbol = " + rule_j.lookahead + " in rule " + rule_j.toString(true));
            }
    ...
    computeStateKernels: function() {
      var init_rule = this.rules[this.start][0].withLookahead(EOF).withFlags(undefined);
      ...
      // console.log(kernelStates.toString());
      // Step 2
      this.states = [];
      var spontLookaheads = {};
      var propLookaheads = {};
      ...
      function printSets(k, v) {
        if (v instanceof KeyedSet || v instanceof IntSet)
          return v.toString();
        else
          return v;
      }
      ...
      for (var i = 0; i < kernelStates.size(); i++) {
        ...
        for (var j = 0; j < kernelState_i.size(); j++) {
          var rule_j = kernelState_i.get(j);
          var rule_j_no_look = rule_j.withLookahead(undefined);
          var rule_j_no_flags = rule_j_no_look.withFlags(undefined);
          // if (rule_j.id !== rule_j_no_look.id) {
          //   console.log("FYI: rule_j is " + rule_j.id + rule_j.toString(true) + " and i = " + i);
          // }
          var lookahead = allLookaheads[i][rule_j_no_flags.id];
          if (lookahead) {
            for (var k = 0; k < lookahead.size(); k++)
              state_i.add(RuleFactory.make(rule_j.name, rule_j.symbols, lookahead.get(k), rule_j.position, rule_j.action, rule_j.flags));
          // } else if (rule_j.symbols[rule_j.position] !== undefined) {
          //   console.log("FYI: filling in for rule_j's lookahead with its next token: " + rule_j.toString(true));
          //   state_i.add(RuleFactory.make(rule_j.name, rule_j.symbols, rule_j.symbols[rule_j.position], rule_j.position, rule_j.action, rule_j.flags));
          // } else if (rule_j.lookahead !== undefined) {
          //   console.log("FYI: filling in with rule_j's lookahead: " + rule_j.toString(true));
          //   state_i.add(RuleFactory.make(rule_j.name, rule_j.symbols, rule_j.symbols[rule_j.position], rule_j.position, rule_j.action, rule_j.flags));
          }
        }
      }
      // Consolidation?
      // var newStates = new SetOfSets(this.states, KeyedSet.equals);
      // var used = {0: true};
      // for (var i = 0; i < this.states.length; i++) {
      //   var state_i = this.states[i];
      //   var existingIndex = newStates.indexOf(state_i);
      //   console.log("State " + i + ": " + state_i.toString());
      //   console.log("Remaps to index " + existingIndex);
      //   for (var row in this.rnTable) {
      //     var tableRow = this.rnTable[row];
      //     for (var atom in tableRow) {
      //       var tableCell = tableRow[atom];
      //       if (tableCell.push === i) {
      //         console.log("Updating " + row + ", " + atom + " from " + tableCell.push + " to " + existingIndex);
      //         used[existingIndex] = true;
      //         tableCell.push = existingIndex;
      //       }
      //     }
      //   }
      // }
      ...
      // for (var row in this.rnTable) {
      //   if (used[row] === true)
      //     newRNTable[row] = this.rnTable[row];
      // }
      // this.rnTable = newRNTable;
      // console.log("This.states = " + this.states.toString());
      // console.log("There are " + this.states.length + " states");
      this.states = new SetOfSets(this.states, KeyedSet.equals); //newStates; // 
      // console.log("After resetting,");
      // console.log("This.states = " + this.states.toString());
      // console.log("There are " + this.states.size() + " states");
    },
    ...
    applyLookaheads: function(closureCache, spont, prop, rule_set, set_num, symbol) {
      ...
      for (var i = 0; i < rule_set.size(); i++) {
        ...
        var jPrime_rule = RuleFactory.make(rule.name, rule.symbols, HASH, rule.position, rule.action, rule.flags);
        ...
        for (var j = 0; j < jPrime.size(); j++) {
          ...
          if (rule_j.position < rule_j.symbols.length && rule_j.symbols[rule_j.position].toString() == symbol) {
            if (rule_j.lookahead !== HASH) {
              var new_rule = RuleFactory.make(rule_j.name, rule_j.symbols, undefined, rule_j.position + 1, rule_j.action, rule_j.flags);
